...
const split=text=>{
 const a=[];let s=String(text||'').trim();
 while(s.length>195){
  let i=s.lastIndexOf(' ',195);
  if(i<80)i=195;
...
export default function AriaVoice({onResponse}){
 ...
 const speak=async(text,h)=>{
  const r=await fetch('/api/aria/voice/speak',{
   method:'POST',
   headers:{...h,'Content-Type':'application/json'},
   body:JSON.stringify({text})
  });
  if(!r.ok)throw Error('ARIA could not speak.');
 ...
 const process=async blob=>{
  ...
  try{
   ...
   if(!activated&&!match){
    await speak('Say Neyo when you are ready.',h);
    return;
   }
   ...
   if(match){
    ...
    if(!message){
     await speak('I’m listening.',h);
     return;
    }
   }

   const cr=await fetch('/api/aria/chat',{
    method:'POST',
    headers:{...h,'Content-Type':'application/json'},
    body:JSON.stringify({
     message,
 ...
 return<div style={box}>
  ...
  <div style={sub}>
   {recording?'Listening…':busy?'ARIA is thinking…':activated?'ARIA is ready for your next command.':'Say “Neyo” to activate ARIA.'}
  </div>

  <div style={actions}>
   {recording?
    <>
     <button style={primary} onClick={stop}>Stop</button>
     <button style={ghost} onClick={cancel}>Cancel</button>
    </>:
    <button style={primary} disabled={busy} onClick={start}>
     {busy?'Working…':'🎙 Talk'}
    </button>
   }
  </div>
