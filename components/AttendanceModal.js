...
const load=useCallback(async(showLoading=true)=>{if(loadInFlight.current)return;const seq=++loadSeq.current;loadInFlight.current=true;if(showLoading&&mounted.current)setLoading(true);if(mounted.current)setError('');try{const s=await auth();if(!s)throw Error('You must be logged in.');const h={Authorization:'Bearer '+s.access_token};const sr=await fetch('/api/attendance/active-session',{headers:h,cache:'no-store'}),sd=await readJson(sr);if(!sr.ok)throw Error(sd.error||'Could not load attendance session.');if(!sd.active){if(seq===loadSeq.current&&mounted.current){setSession(null);setCanDiscard(false);setPeople([]);setQuery('')}return}const ns=normalizeSession(sd);if(!ns)throw Error('Attendance session response was incomplete.');if(ns.status==='closed'){if(seq===loadSeq.current&&mounted.current){setSession(null);setCanDiscard(false);setPeople([]);setQuery('');setError(ns.processing_status==='failed'?'ARIA could not finish the previous attendance. You can start a new session.':'Attendance processing is still finishing. You can start a new session.')}return}const pr=await fetch('/api/attendance/people?session_id='+encodeURIComponent(ns.session_id),{headers:h,cache:'no-store'}),pd=await readJson(pr);if(!pr.ok)throw Error(pd.error||'Could not load attendance people.');if(seq!==loadSeq.current||!mounted.current)return;setSession(ns);setCanDiscard(ns.can_discard===true);setPeople(normalizePeople(pd));}catch(e){console.error('[ATTENDANCE]',e);if(seq===loadSeq.current&&mounted.current)setError(String(e?.message||'Could not load attendance.'))}finally{loadInFlight.current=false;if(showLoading&&mounted.current)setLoading(false)}},[]);
