...
<div style={processingCompact}>
<div style={processingIndicator}><i/></div>
<div style={processingCopy}>
<strong style={processingTitle}>{ariaProcessingFailed?'ARIA needs another pass':'ARIA is finalizing this attendance'}</strong>
<span style={processingText}>{ariaProcessingFailed?'The attendance is safely saved. Retry ARIA processing to unlock the next session.':'Attendance is saved. ARIA is working in the background. You can close this window; NYEOCARE will unlock the next session automatically when the pass finishes.'}</span>
</div>
</div>
...
const noticeBox={margin:'8px 18px 0',padding:'10px 12px',borderRadius:12,background:'rgba(255,255,255,.055)',border:'1px solid rgba(255,255,255,.1)',color:'rgba(255,255,255,.82)',fontSize:13},processingBox={flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:12,padding:'34px 26px',textAlign:'center',maxWidth:700,margin:'0 auto'},processingTitle={fontSize:16,margin:0},processingText={fontSize:12,lineHeight:1.5,color:'rgba(255,255,255,.52)',display:'block'},processingCompact={display:'flex',alignItems:'flex-start',gap:12,width:'min(640px,100%)',padding:'16px 18px',borderRadius:18,background:'rgba(255,255,255,.045)',border:'1px solid rgba(255,255,255,.09)'},processingIndicator={width:10,height:10,borderRadius:'50%',background:'rgba(255,255,255,.62)',marginTop:5,boxShadow:'0 0 0 5px rgba(255,255,255,.055)'},processingCopy={display:'flex',flexDirection:'column',gap:4,textAlign:'left',flex:1},overlay={position:'fixed',inset:0,zIndex:2147483000,background:'rgba(2,5,12,.68)',backdropFilter:'blur(18px)',display:'flex',alignItems:'center',justifyContent:'center',padding:12,overflow:'auto'};
