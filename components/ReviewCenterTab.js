...
const r=await fetch('/api/identity/review-items',{headers:{Authorization:`Bearer ${session.access_token}`}});
...
if(!confirm(type==='confirm'?'Use the suggested identity for this scan?':'Keep this as a new person?'))return;
...
const r=await fetch('/api/identity/review-action',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${session.access_token}`},body:JSON.stringify(body)});
const d=await r.json();
...
const candidate={margin:'18px 0 12px;padding:14px;borderRadius:18,border:'1px solid rgba(143,183,255,.13)',background:'rgba(143,183,255,.045)',display:'flex',justifyContent:'space-between',alignItems:'center',gap:12};
