...
function compactPeople(parsed){
 ...
 return rows.map(x=>{
  ...
  const name=normalizeDisplayName(String(x?.n??x?.name??rawName??'').trim())||null;
  return{
   ...
   confidence,
   name_confidence:Number(x?.nc??x?.name_confidence??confidence)||confidence,
   phone_confidence:Number(x?.pc??x?.phone_confidence??confidence)||confidence,
...
function prompt(recovery=false){
 return`Read the ORIGINAL photographed attendance register carefully. This is semantic document understanding and handwriting recognition, not blind OCR. Understand the physical rows and columns before extracting people.
Extract every real person. Ignore headers, totals, dates, notes, signatures and non-person text.
For each person:
- n = clean human-readable name casing. Example: "isahea EDwaRD" becomes "Isahea Edward".
- o = exact name appearance as visibly written.
- p = phone digits exactly as visible; never invent missing digits.
- Pair a phone only when the page visibly supports the relationship.
...
- Do not use outside knowledge to repair unreadable handwriting.
- Preserve honorifics only when they are genuinely part of the person's written identity.
Return ONLY compact JSON:
{"people":[{"r":1,"n":"Isahea Edward","o":"isahea EDwaRD","p":"08039579788","c":95,"rel":"same_row","e":null}]}
c is extraction confidence for the complete name, phone and physical pairing. It is NOT an acceptance threshold.
...
${recovery?'Re-read the ORIGINAL IMAGE from the beginning. Return every person you can safely extract. Do not omit rows simply because they are uncertain. JSON only.':''}`;
}
