...
const OUTPUT_MAX=Math.min(1600,MODEL.max_completion_tokens||1600);
const RECOVERY_MAX=900;
const TIMEOUT=90000,MAX_RETRIES=2,PIPELINE_VERSION='v14-compact-name-phone-50';
...
function compactPeople(parsed){
 ...
 return rows.slice(0,MAX_PEOPLE_PER_PAGE).map(x=>{
  ...
  return{name,raw_name:String(x?.o??x?.raw_name??x?.n??x?.name??'').trim()||name,phone:phones[0]||null,phones,confidence:0,name_confidence:0,phone_confidence:0,pair_confidence:0,row_number:null};
 }).filter(x=>x.name||x.phone);
}

function prompt(recovery=false){
 return`Read the ORIGINAL photographed attendance register page. Extract every real person on THIS PAGE, up to 50 people maximum.

Return ONLY compact JSON:
{"people":[{"n":"Name","p":["08012345678"]}]}

Rules:
- n = the person's written name, cleaned only for obvious spacing/capitalization.
- p = every phone number visibly assigned to that person, normally 0, 1 or 2 numbers.
- Read the actual digits. NEVER invent, autocomplete, repair or substitute a digit.
- Preserve the number exactly as written, including leading 0 or +234.
- A phone belongs to a name only when the page visually supports the assignment through the same row or an obvious arrow/line/continuation.
- Do not assign a nearby number merely because it is close.
- If a phone digit is genuinely unreadable, omit that phone rather than guess.
- Ignore headers, totals, dates, notes and signatures.
- Do not return explanations, confidence, row numbers, relationships or reasoning.
- Never return more than 50 people.
${recovery?'Re-read the original page once and return ONLY the compact JSON. Focus on exact phone digits and name/phone association. Do not guess.':''}`;
}
...
async function request(imageBase64,text,maxTokens,options,attempt){
 ...
 try{
  const body={model:MODEL.model,messages:[{role:'user',content:[{type:'text',text},{type:'image_url',image_url:{url:`data:image/jpeg;base64,${imageBase64}`}}]}],temperature:.1,top_p:.8,max_completion_tokens:maxTokens,response_format:{type:'json_object'}};
  ...
  if(finishReason==='length')throw Object.assign(new Error('Groq response reached its output limit'),{code:'AI_OUTPUT_TRUNCATED',retryable:false,outputTruncated:true});
...
function normalizeResult(result,mode){
 ...
 const extractionQuality=people.length?100:0;
...
export async function callVisionWithRetry(imageBase64,_unused,onProgress=null,options={}){
 ...
 for(let attempt=1;attempt<=MAX_RETRIES;attempt++)try{
  ...
  lastError=err;
  if(err?.code==='AI_OUTPUT_TRUNCATED')throw err;
  if(attempt>=MAX_RETRIES||!err?.retryable)throw err;
