...
async function waitForCapacity(required){const g=gate();if(!g||g.remainingTokens===null||Date.now()>=Number(g.resetAt||0)||g.remainingTokens>=required)return;const wait=Math.min(Math.max(Number(g.resetAt)-Date.now(),1000),90000);await new Promise(r=>setTimeout(r,wait))}
export function getScanAdmission(){if(!GROQ_API_KEY)return{allowed:false,code:'AI_NOT_CONFIGURED',message:'ARIA scan is not configured yet.'};const g=gate();if(g?.remainingTokens!==null&&Date.now()<Number(g.resetAt||0)&&g.remainingTokens<COMBINED_HEADROOM)return{allowed:false,code:'AI_CAPACITY_LOW',message:'ARIA is waiting for enough provider capacity. Please try again shortly.',retry_after_seconds:Math.ceil((Number(g.resetAt)-Date.now())/1000)};return{allowed:true,required_output_tokens:EXTRACT_MAX+AUDIT_MAX,safety_tokens:SAFETY}}
...
const latency=Date.now()-started;setGate(response.headers);
...
const admission=getScanAdmission();if(!admission.allowed)throw Object.assign(new Error(admission.message),{status:429,retryable:false,code:admission.code,retryAfter:admission.retry_after_seconds});
...
lastError=err;if(!err.retryable||attempt>=2)throw err;
...
onProgress?.('retrying',delay);await new Promise(r=>setTimeout(r,delay));
}}
