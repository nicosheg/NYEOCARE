...
const MAX_PEOPLE_PER_PAGE=50,OUTPUT_MAX=Math.min(1600,MODEL.max_completion_tokens||1600),RECOVERY_MAX=900,TIMEOUT=90000,MAX_RETRIES=2,PIPELINE_VERSION='v15-clean-names-review';
...
function compactPeople(parsed){
 ...
 return rows.slice(0,MAX_PEOPLE_PER_PAGE).map(x=>{
  const name=cleanName(x?.n??x?.name??'');
...
function prompt(recovery=false){
 return`Read the ORIGINAL photographed attendance register page. Extract every real person on THIS PAGE, maximum 50 people.

Return ONLY:
{"people":[{"n":"Name","p":["08012345678"]}]}

Rules:
- n is the person's written name.
- Preserve all meaningful spaces between names and words.
- p contains every phone number visibly assigned to that person, normally 0, 1 or 2.
- Read the actual digits. NEVER invent, autocomplete, repair or substitute digits.
...
${recovery?'Re-read the original page and return the compact JSON only. Focus especially on exact phone digits and name/phone pairing. Never guess.':''}`;
}
