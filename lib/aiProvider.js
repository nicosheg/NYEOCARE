...
const CONFIGURED_CEILING=Number(process.env.GROQ_SCAN_OUTPUT_CEILING||1000);
const PROVIDER_OUTPUT_CEILING=Number.isFinite(CONFIGURED_CEILING)&&CONFIGURED_CEILING>0?CONFIGURED_CEILING:1000;
...
const MIN_OUTPUT=600,DEFAULT_OUTPUT=800,RECOVERY_MAX=Math.min(700,SAFE_OUTPUT_CEILING-50);
const MAX_RETRIES=2,TIMEOUT=90000,PIPELINE_VERSION='v18-adaptive-scan';
...
function compactPeople(parsed){
 ...
 return rows.slice(0,MAX_PEOPLE_PER_PAGE).map(x=>{
  ...
  return{name,raw_name:name,phone:phones[0]||null,phones,rawPhones:phones,confidence:0,name_confidence:0,phone_confidence:0,pair_confidence:0,row_number:null};
...
${recovery?'Re-read the ORIGINAL PAGE carefully. Prioritize exact digits and exact name-to-phone pairing. If uncertain, omit rather than guess.':''}`;
}
...
function estimateOutputBudget(imageBase64){
 ...
 if(kb>=1200)budget=900;
 else if(kb>=900)budget=880;
 else if(kb>=650)budget=850;
 else if(kb>=450)budget=800;
 else if(kb>=250)budget=750;
 else budget=700;
...
async function request(imageBase64,text,maxTokens,options,attempt){
 ...
 try{
  ...
  if(finishReason==='length')throw Object.assign(new Error('ARIA could not safely finish reading this register.'),{code:'AI_OUTPUT_TRUNCATED',retryable:false});
...
export async function callVisionWithRetry(imageBase64,_unused,onProgress=null,options={}){
 ...
 for(let attempt=1;attempt<=MAX_RETRIES;attempt++)try{
  ...
  const vision=await request(imageBase64,prompt(false),outputBudget,options,attempt);
  ...
  const recoveryBudget=Math.max(MIN_OUTPUT,Math.min(RECOVERY_MAX,outputBudget-100));
