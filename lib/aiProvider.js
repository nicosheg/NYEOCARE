...
return`You are ARIA, digitising a photographed handwritten attendance register. Read the ORIGINAL IMAGE itself. Accuracy is more important than completeness, but return every person you can safely identify.

Understand the page before extracting it. Identify the real register rows and the name/phone columns. The page may be tilted, crooked, crowded, folded or uneven. Do not assume perfect horizontal alignment.

PAIRING RULE: a phone belongs to a name only when the image supports that relationship. Same-row placement is strongest. Also follow a clearly visible arrow, bracket, continuation mark, line or other explicit visual connection. Never pair by array position, proximity alone, or a guessed correction.

NAME: preserve the written spelling and visible title such as Sis, Bro, Mrs, Pastor, Dr, Rev, Elder or Deacon. Never replace an unusual name with a common one.
PHONE: read every visible digit. Do not invent, autocorrect or normalize digits inside the extraction. If a digit is genuinely unreadable, use null.
IGNORE headers, totals, notes, signatures, dates and non-person text. Never split one person's continued entry or merge two different people.

For each real person return compact fields: r=logical row number, n=name or null, p=phone as visibly written or null, c=0-100 confidence for the complete name+phone+pairing, rel=one of same_row, arrow_link, continuation, visual_link, uncertain, e=null for same_row otherwise a very short visible-link description.

Return JSON only in exactly this shape: {"people":[{"r":1,"n":"...","p":"...","c":98,"rel":"same_row","e":null}]}. Do not add prose.${fallback?' Re-read the image from scratch because the previous extraction found no usable people.':''}`
}
function normalizePerson(row){
const name=String(row?.n??row?.name??'').trim()||null;
const phone=String(row?.p??row?.phone??'').trim()||null;
const confidence=Math.max(0,Math.min(100,Number(row?.c??row?.confidence??row?.pair_confidence)||0));
const relation=String(row?.rel??row?.phone_relation??'uncertain');
const evidence=row?.e??row?.link_evidence??null;
return{row_number:Number.isInteger(Number(row?.r??row?.row_number))?Number(row?.r??row?.row_number):null,name,phone,name_confidence:confidence,phone_confidence:confidence,pair_confidence:confidence,phone_relation:relation,link_evidence:evidence==null?null:String(evidence).trim()||null}
}
...
try{
const body={model:config.model,messages:[{role:'user',content:[{type:'text',text:prompt},{type:'image_url',image_url:{url:`data:image/jpeg;base64,${imageBase64}`}}]}],temperature:.2,top_p:.9,max_completion_tokens:maxCompletionTokens,response_format:{type:'json_object'}};
