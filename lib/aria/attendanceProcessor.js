...
async function getSession(sessionId, orgId, db = pool) {
  const r = await db.query(
    `SELECT id,name,service_type,started_at,status,aria_processing_status
...
latest AS (
 SELECT s.id,s.name,s.service_type,s.started_at,
...
SELECT * FROM candidates`;
  ...
  try {
    ...
    if(candidates.rows.length){
      await client.query(
        ...
        SELECT $1,c.person_id,'UNUSUAL_ABSENCE',1,'low','low',16,
          jsonb_build_object(
            ...
            'observation_semantics','absence_observation',
      ...
      await client.query(
        `INSERT INTO aria_events(organization_id,person_id,type,source,event_key,metadata,occurred_at,evidence_kind,confidence,verification_status,scope_level,processing_status,processed_at)
         SELECT $1,c.person_id,'PERSON_NOT_OBSERVED','attendance',
                'attendance:not_observed:'||$2::text||':'||c.person_id::text,
                jsonb_build_object('session_id',c.session_id,'session_name',c.session_name,'service_type',c.service_type,'observation_only',true,'care_eligible',true),
                NOW(),'observation',1,'observed','organization','completed',NOW()
         FROM (SELECT * FROM jsonb_to_recordset($2::jsonb) AS x(person_id uuid,session_id uuid,session_name text,service_type text)) c
         ON CONFLICT(organization_id,event_key) DO NOTHING`,
        [orgId,sessionId,JSON.stringify(candidates.rows)]
      );
