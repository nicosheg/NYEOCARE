...
export async function getAttentionSummary(organizationId,{limit=8}={}){
 ...
 const highResult=await pool.query("SELECT COUNT(*)::int count FROM aria_observations WHERE organization_id=$1 AND status='active' AND severity IN('high','critical') AND(expires_at IS NULL OR expires_at>NOW())",[organizationId]);
 const openActions=Object.values(briefing.intelligence?.action_inventory||{}).reduce((sum,v)=>sum+Number(v||0),0);
 return{
  items,
  counts:{active_observations:activeObservations,high_observations:Number(highResult.rows[0]?.count)||0,open_actions:openActions},
