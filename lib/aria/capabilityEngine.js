...
async function personContext(organizationId,personId){
 const[person,intelligence,state,memory,timeline,feedback,interaction]=await Promise.all([
  ...
  pool.query(`SELECT kind,occurred_at FROM(SELECT 'communication' kind,pc.occurred_at FROM person_communications pc WHERE pc.organization_id=$1 AND pc.person_id=$2 AND pc.status NOT IN('draft','failed','cancelled') UNION ALL SELECT 'care_feedback',cf.observed_at FROM care_feedback cf WHERE cf.organization_id=$1 AND cf.person_id=$2 UNION ALL SELECT 'attendance_context',ac.created_at FROM aria_attendance_contexts ac WHERE ac.organization_id=$1 AND ac.person_id=$2 AND ac.source='human' UNION ALL SELECT 'timeline',te.occurred_at FROM timeline_events te WHERE te.people_id=$2 AND COALESCE(te.source,'')='human' AND te.event_type NOT IN('identity_review','aria_draft','person_archived','person_updated')) recent ORDER BY occurred_at DESC LIMIT 1`,[organizationId,personId])
 ]);
