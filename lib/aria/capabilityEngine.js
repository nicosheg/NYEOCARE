...
async function personContext(organizationId,personId){
 const[person,intelligence,state,memory,timeline,feedback,livingTruth]=await Promise.all([
 ...
 return{person:person.rows[0],intelligence:intelligence.rows[0]||null,state:state.rows[0]||null,memory:memory.rows,timeline,feedback:feedback.rows,living_truth:livingTruth};
}
