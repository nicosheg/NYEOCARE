...
async function organizationContext(organizationId){
 const [people,birthdays,tasks,intelligence]=await Promise.all([
 ...
 return {people:people.rows,birthdays:birthdays.rows,followups:tasks.rows,attention:intelligence.rows,counts:{people:people.rows.length,with_birthday:birthdays.rows.length,pending_followups:tasks.rows.length,attention_items:intelligence.rows.length}};
}
