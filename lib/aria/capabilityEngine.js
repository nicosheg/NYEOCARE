...
async function personContext(organizationId,personId){
 const[person,intelligence,state,memory,timeline,feedback]=await Promise.all([
  pool.query(`SELECT p.id,p.first_name,p.last_name,p.display_name,p.phone,p.email,p.type,em.last_seen,
   ...
   WHERE p.id=$1 AND p.organization_id=$2 AND COALESCE(p.status,'active')='active' LIMIT 1`,[personId,organizationId]),
