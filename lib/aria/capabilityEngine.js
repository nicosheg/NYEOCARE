...
import{resolvePeopleCohort,cohortLabel}from'./cohortEngine';
import{createCareDraftBatch}from'./batchDraftEngine';
...
async function observationContext(organizationId,observationId){
 const r=await pool.query(`
  SELECT o.id,o.person_id,o.type,o.status,o.confidence,o.severity,o.urgency,o.attention_score,o.evidence,o.metadata,o.detected_at,o.expires_at,o.resolved_at,
         p.display_name,p.first_name,p.last_name,p.phone,p.email,
         a.id AS action_id,a.type AS action_type,a.status AS action_status,a.priority AS action_priority,a.action_metadata,
         a.proposed_at,a.approved_at
  FROM aria_observations o
  LEFT JOIN people p ON p.id=o.person_id AND p.organization_id=o.organization_id
  LEFT JOIN LATERAL(
    SELECT id,type,status,priority,action_metadata,proposed_at,approved_at
    FROM aria_actions
    WHERE organization_id=o.organization_id AND observation_id=o.id AND status IN('proposed','approved','executing')
 ...
 return{
  observation:{
   id:x.id,person_id:x.person_id,person_name:x.display_name||[x.first_name,x.last_name].filter(Boolean).join(' ')||null,
   type:x.type,status:x.status,confidence:Number(x.confidence)||0,severity:x.severity,urgency:x.urgency,
   attention_score:Number(x.attention_score)||0,evidence:x.evidence||{},metadata:x.metadata||{},
   detected_at:x.detected_at,expires_at:x.expires_at,resolved_at:x.resolved_at
  },
  action:x.action_id?{id:x.action_id,type:x.action_type,status:x.action_status,priority:x.action_priority,metadata:x.action_metadata||{},proposed_at:x.proposed_at,approved_at:x.approved_at}:null
 };
}
...
export async function executeCapability({organizationId,capability,personId=null,personName=null,parameters={},actorId=null}){
 ...
 switch(capability){
  ...
  case'get_director_briefing':
   return{capability,briefing:await getDirectorBriefing(organizationId,{limit:parameters.limit||8})};
  ...
  case'get_observation_context':{
   const observationId=parameters.observationId||parameters.observation_id;
   if(!observationId)throw Object.assign(new Error('observationId required'),{status:400});
   const context=await observationContext(organizationId,observationId);
   if(!context)throw Object.assign(new Error('Observation not found'),{status:404});
   return{capability,observation:context.observation,action:context.action};
  }

  case'resolve_people_cohort':{
   const key=clean(parameters.cohort||parameters.key||'current_attention',120);
   const limit=parameters.all?null:(parameters.limit==null?10:Math.min(Math.max(Number(parameters.limit)||1,1),50));
   const result=await resolvePeopleCohort(organizationId,{key,limit,personIds:parameters.personIds||parameters.person_ids||null,days:parameters.days||30});
   return{capability,cohort:result.key,cohort_label:cohortLabel(result.key),count:result.count,total:result.total,truncated:result.truncated,people:result.people,person_ids:result.people.map(x=>x.person_id)};
  }
  ...
  case'draft_message':{
   ...
   const result=await createCareDraft({
    organizationId,personId,actionId:parameters.actionId||null,
    actionType:parameters.actionType||'thoughtful_check_in',actorId,
    approvedOnly:false,channel:parameters.channel==='whatsapp'?'whatsapp':'app',
    idempotencyKey:parameters.idempotencyKey||null
   });
   return{capability,person_name:personName||null,...result,draft:result};
  }

  case'draft_message_cohort':{
   const key=clean(parameters.cohort||parameters.key||'current_attention',120);
   const cohort=await resolvePeopleCohort(organizationId,{
    key,limit:parameters.all?null:Math.min(Math.max(Number(parameters.limit)||10,1),50),
    personIds:parameters.personIds||parameters.person_ids||null,days:parameters.days||30
   });
   if(!cohort.people.length)return{capability,cohort:cohort.key,count:0,total:0,truncated:false,drafts:[],message:'I could not find any people matching that group.'};
   const batch=await createCareDraftBatch({
    organizationId,
    people:cohort.people,
    actionType:parameters.actionType||'thoughtful_check_in',
    actorId,
    channel:parameters.channel==='whatsapp'?'whatsapp':'app',
    idempotencyKey:parameters.idempotencyKey||null
   });
   return{capability,cohort:cohort.key,cohort_label:cohortLabel(cohort.key),count:cohort.people.length,total:cohort.total,truncated:cohort.truncated,...batch};
  }
