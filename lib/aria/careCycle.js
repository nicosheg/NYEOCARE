...
export async function runCareCycle(orgId,{force=false}={}){
 ...
 let failed=0;
 if(!ids.length){let due=0;try{due=await generateReturnDueActions(orgId)}catch(e){failed++;console.error('[ARIA] return-due',e.message)}return{organizationId:orgId,processed:0,failed,actions:due,ranAt:new Date().toISOString()};}
 for(const id of ids){
