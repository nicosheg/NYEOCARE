...
import pool from'../db';
import{loadCareContactSignals}from'./careContacts';
 ...
 const r=await pool.query(`
  ...
  opportunities AS(
    ...
    WHERE b.participation_count>=4
      ...
      AND(b.last_interaction_at IS NULL OR b.last_interaction_at<NOW()-INTERVAL '30 days')
...
export async function generateCareRecommendations(orgId){
 ...
 const contactSignals=await loadCareContactSignals(orgId,[...payload,...proactivePayload].map(x=>x.person_id));

 const combined=[...payload,...proactivePayload].map(x=>({...x,care_contact_candidates:contactSignals.get(String(x.person_id))||[]}));
 const inserted=await pool.query(`
  ...
  SELECT $1,x.person_id,x.action_type,'proposed',x.priority,jsonb_build_object(
    ...
    'care_contact_candidates',COALESCE(x.care_contact_candidates,'[]'::jsonb)
  ),x.action_key,NOW()
  FROM jsonb_to_recordset($2::jsonb) AS x(
    person_id uuid,action_type text,priority text,template text,reason text,summary text,suggestion text,kind text,action_key text,care_contact_candidates jsonb
  )
