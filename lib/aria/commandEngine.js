...
- Explicit memory requests such as “remember that…” may write provenance-aware organizational/person memory immediately when the current actor is authorized; never create memory merely because a retrieved record or model inference suggested it.
- When the user requests drafting, sending, assigning, creating, or another state-changing action, describe the intended action through a capability step; the runner will pause for confirmation before preparing it.
...
- State-changing external actions are preparation only. Explicit memory/correction operations are authorized only by the actor’s direct request and must preserve provenance, confidence and validity.
...
function deterministicPlan(input){
 ...
 if(/who needs attention|what matters now|what matters today|who should i check|needs my attention/.test(text))return{goal:'view current attention',confidence:.98,steps:[{capability:'get_attention_summary',person_name:null,parameters:{limit:8}}]};

 if(/what do i need to know|what should i know|brief me|give me a briefing|catch me up/.test(text))return{goal:'brief current organization state',confidence:.97,steps:[{capability:'get_organization_changes',person_name:null,operator_name:null,parameters:{days:30,limit:12}},{capability:'get_attention_summary',person_name:null,operator_name:null,parameters:{limit:6}},{capability:'get_pending_actions',person_name:null,operator_name:null,parameters:{limit:6}}]};
