...
export async function runCommand({organizationId,userId,message,history=[],conversationId=null,defaultPersonId=null,defaultPersonName=null}){
 const input=clean(message);
 ...
 for(const step of plan.steps){
  ...
  if(capability.approval){
   ...
   const action=await planActionFromObservation({
    ...
    actionMetadata,
    actionKey:`aria-chat:${conversationId||'none'}:${Date.now()}:${actionType}`
   });
