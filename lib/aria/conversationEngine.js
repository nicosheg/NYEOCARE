...
async function resolveConversationalFollowUp({organizationId,userId,input,state,defaultPersonId=null,defaultPersonName=null}){
 ...
 if(DRAFT_FOLLOW_UP.test(text)){
  ...
  if(onlyName){
   ...
   if(matches.results?.length>1)return{type:'clarification',text:'I found more than one person matching that name. Tell me which one you mean and I will draft only for that person.',plan:{goal:'resolve one draft person',confidence:1,steps:[]},results:[matches]};
  }
