...
async function naturalResponse({organizationId,message,result,history=[]}){
 ...
 const needsNarrative=/\\b(what should i do next|what do i do next|what matters most|what needs my attention|why|explain|interpret|compare|should i|how should i|what does this mean|help me decide|think through|talk me through)\\b/i.test(message);
 if(direct&&!needsNarrative)return direct;
 const system=`${ARIA_CORE_PERSONALITY}\\n\\nSpeak naturally and briefly, but always finish the thought. Use only supplied results. Never invent facts. Never claim an action happened when it was only prepared. Never expose implementation details. If something requires approval, say so clearly. Use Markdown when it improves readability: headings, numbered or bulleted lists, bold emphasis and italics are welcome. Keep the response focused and normally under 700 output tokens. Never deliberately end on an incomplete sentence.`;
