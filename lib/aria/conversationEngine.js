...
function summarizeResult(result){
 ...
 for(const item of result.results||[]){
  if(item.capability==='find_person'){
   ...
   else parts.push('I found '+people.length+' matching '+(people.length===1?'person.':'people.')+(people.length<=4?' '+people.map(p=>p.display_name||[p.first_name,p.last_name].filter(Boolean).join(' ')).join(', '):'');
  }
