...
function suggestedActionFromResult(result,personId){
 ...
 for(const item of result.results||[]){
  if(item.capability==='prepare_message'&&item.draft){
   return{personId:personId||item.action?.person_id||null,actionId:item.action?.id||null,actionType:'SEND_MESSAGE',label:'Open WhatsApp',reason:'The draft is ready for your review. Nothing has been sent.',approved:true,requiresApproval:false,status:'approved'};
  }
