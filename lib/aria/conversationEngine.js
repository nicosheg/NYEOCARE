...
function summarizeResult(result){
 ...
 for(const item of result.results||[]){
  ...
  if(item.capability==='get_organization_changes'){const h=item.highlights||[],c=item.counts||{};parts.push(h.length?'I found '+h.length+' recent organization changes in the loaded window.':'I found no recent organization changes to surface.');if(c.people_added_7d!==undefined)parts.push('Recent 7-day activity: '+c.people_added_7d+' people added, '+c.sessions_7d+' sessions, '+c.participation_7d+' confirmed participation records, and '+c.feedback_7d+' care feedback records.');if(h[0])parts.push('Most recent: '+String(h[0].label||h[0].kind)+' — '+String(h[0].detail||'').trim()+'.');}
  if(item.capability==='get_attention_summary'){const items=item.items||[],c=item.counts||{};parts.push(items.length?'ARIA has '+items.length+' current attention signal'+(items.length===1?'':'s')+' worth review.':'ARIA has no current high-value attention signal to surface.');if(items[0])parts.push('One signal: '+(items[0].person_name||'Someone')+' — '+(items[0].reason||'review the available context')+'.');}
...
export async function handleConversation({organizationId,message,history=[],conversationId=null,userId=null,personId=null}){
 ...
 let text;
 try{text=await naturalResponse({organizationId,message:input,result,history:safeHistory});}
