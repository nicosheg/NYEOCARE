...
async function naturalResponse({organizationId,message,result,history=[]}){
 ...
 try{
  const ai=await generateText({
   ...
   maxTokens:450,
   temperature:.25,
   system:`You are ARIA inside NYEOCARE. Speak naturally and briefly. Use only supplied results. Never invent facts. Never claim an action happened when it was only prepared. Never expose implementation details. If something requires approval, say so clearly.`,
   messages:[
    ...history.slice(-8).map(x=>({role:x.role==='assistant'?'assistant':'user',content:clean(x.content,2500)})),
...
export async function handleConversation({organizationId,message,history=[],conversationId=null,userId=null}){
 ...
 if(conversationId){
  const existing=await pool.query(
   `SELECT id FROM aria_conversations WHERE id=$1 AND organization_id=$2 AND status='active' LIMIT 1`,
  ...
  conversation=(await pool.query(
   `INSERT INTO aria_conversations(organization_id,user_id,status) VALUES($1,$2,'active') RETURNING id`,
   [organizationId,userId]
  )).rows[0];
 }

 const persisted=await pool.query(
  `SELECT role,content FROM aria_messages WHERE conversation_id=$1 ORDER BY created_at DESC LIMIT 20`,
  [conversation.id]
 );
 ...
 await pool.query(
  `INSERT INTO aria_messages(conversation_id,role,content,metadata) VALUES($1,'user',$2,'{}')`,
 ...
 await pool.query(
  `INSERT INTO aria_messages(conversation_id,role,content,metadata) VALUES($1,'assistant',$2,$3)`,
  [conversation.id,text,{result_type:result.type||null}]
 );

 await pool.query(
  `UPDATE aria_conversations SET updated_at=NOW() WHERE id=$1 AND organization_id=$2`,
