...
function suggestedActionFromResult(result,personId){
 ...
 for(const item of result.results||[]){
  if(item.capability==='get_person_context'||item.capability==='explain_person'){
   ...
   if(a)return{personId,actionType:a==='SEND_MESSAGE'?'SEND_MESSAGE':'REQUEST_REVIEW',actionKind:a,label:a==='SEND_MESSAGE'?'Prepare a check-in':'Review next step',reason:item.intelligence?.action_reason||'ARIA identified a meaningful next step.'};
   const pending=item.pending_actions?.find(x=>x.status==='proposed'||x.status==='approved');
   if(pending)return{personId,actionId:pending.id,actionType:pending.type,label:pending.type==='SEND_MESSAGE'?'Prepare message':'Review action',reason:pending.action_metadata?.reason||'ARIA has a prepared next step.'};
  }
  if(item.capability==='get_care_recommendations'&&item.recommendations?.[0]){
   const r=item.recommendations[0];
   return{personId:r.person_id,actionId:r.action_id||null,actionType:r.action_type||'SEND_MESSAGE',label:r.action_type==='SEND_MESSAGE'?'Prepare message':'Review next step',reason:r.care_reason||r.reason||'ARIA identified a care opportunity.'};
  }
 }
 return null;
}

function summarizeResult(result){if(!result)return'';if(result.type==='clarification'||result.type==='action_prepared')return result.text||'';if(result.type==='completed'){const parts=[];for(const item of result.results||[]){if(item.capability==='find_person')parts.push(`Found ${item.results?.length||0} matching people.`);if(item.capability==='get_pending_actions')parts.push(`There are ${item.actions?.length||0} pending actions.`);if(item.capability==='get_care_recommendations')parts.push(`There are ${item.recommendations?.length||0} care opportunities.`);if(item.capability==='get_person_context'||item.capability==='explain_person')parts.push('I retrieved the person’s current context.');if(item.capability==='prepare_message')parts.push('A message has been prepared for review.');if(item.capability==='prepare_action')parts.push('An action has been prepared for approval.')}return parts.join(' ')}return''}
async function naturalResponse({organizationId,message,result,history=[]}){const direct=summarizeResult(result);if(result.type==='clarification'||result.type==='action_prepared')return direct;try{const ai=await generateText({organizationId,purpose:'aria_conversation_response',maxTokens:450,temperature:.25,system:`${ARIA_CORE_PERSONALITY}\n\nSpeak naturally and briefly. Use only supplied results. Never invent facts. Never claim an action happened when it was only prepared. Never expose implementation details. If something requires approval, say so clearly.`,messages:[...history.slice(-8).map(x=>({role:x.role==='assistant'?'assistant':'user',content:clean(x.content,2500)})),{role:'user',content:`Request: ${message}\nResult: ${JSON.stringify(result).slice(0,18000)}`}]});return clean(ai.text,5000)}catch{return direct||'I completed the part of that request I could safely process.'}}
