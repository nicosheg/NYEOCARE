...
export const ARIA_DIRECTOR_VERSION='3.0.0';
...
export async function getDirectorContext({organizationId,viewerId,personId=null}={}){if(!organizationId||!viewerId)throw Object.assign(new Error('Director context requires an authenticated operator.'),{status:401});const[changes,attention,truth]=await Promise.all([getOrganizationChanges(organizationId,{days:30,limit:20}),getAttentionSummary(organizationId,{limit:8}),personId?getLivingTruth({organizationId,personId}):Promise.resolve(null)]);return{director:'ARIA',version:ARIA_DIRECTOR_VERSION,generated_at:new Date().toISOString(),changes,attention,living_truth:truth};}
...
export async function getDirectorState(organizationId){if(!organizationId)throw new Error('organizationId required');const[rules,scan,session,observations,actions,events,changes,attention]=await Promise.all([
 ...
 getOrganizationChanges(organizationId,{days:30,limit:12}),
 getAttentionSummary(organizationId,{limit:6})
 ]);return{director:'ARIA',version:ARIA_DIRECTOR_VERSION,people:rules.rows[0].count,pending_scan_reviews:scan.rows[0].count,latest_session:session.rows[0]||null,active_observations:observations.rows[0].count,open_actions:actions.rows[0].count,last_event_at:events.rows[0]?.last_event_at||null,recent_changes:changes,attention:attention}}
