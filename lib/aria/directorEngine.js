...
export const ARIA_DIRECTOR_CORE_VERSION = '4.1.0';

const DIRECTOR_CACHE_TTL_MS=2000;
const directorCache=new Map();
const directorInflight=new Map();
...
export function reconcileAttendanceCandidate(candidate) {
...
async function loadCore(orgId) {
  const [population,sessions,knowledge,operations,observations,actions,feedback,queue,recentSignals,orgMemory] = await Promise.all([
    pool.query(
      ...
      "(SELECT COUNT(DISTINCT ar.people_id)::int FROM attendance_records ar WHERE ar.organization_id=$1 AND ar.attendance_date>=CURRENT_DATE-INTERVAL '30 days' AND ar.present=true) active_attendees_30d " +
      "FROM people WHERE organization_id=$1 AND COALESCE(status,'active')='active'",
      [orgId]
    ),
    pool.query(
      "SELECT " +
      "COUNT(*) FILTER(WHERE started_at>=CURRENT_DATE)::int sessions_today," +
    ...
    pool.query(
      ...
      "(SELECT COUNT(*)::int FROM relationship_scores rs WHERE rs.organization_id=$1) relationships_ready," +
    ...
    pool.query(
      ...
      "(SELECT COUNT(*)::int FROM aria_actions a WHERE a.organization_id=$1 AND a.status IN('proposed','approved','executing') AND a.priority IN('critical','high') AND COALESCE(a.action_metadata->>'kind','') IN('scan_review_required','aria_processing_failure','system_integrity','security_review','data_integrity')) high_priority_operational_actions",
      [orgId]
    ),
    pool.query(
      ...
      "GROUP BY type ORDER BY count DESC LIMIT 100",
      [orgId]
    ),
    pool.query(
  ...
  return {
    ...
    operations:{
      ...(operations.rows[0]||{}),
      active_observations:observations.rows.reduce((sum,row)=>sum+n(row.count),0),
      meaningful_observations:observations.rows
        .filter(row=>['UNUSUAL_ABSENCE','SCAN_REVIEW_REQUIRED','ARIA_PROCESSING_FAILURE','CONFLICT','DATA_INTEGRITY','SECURITY_REVIEW'].includes(String(row.type||'')))
        .reduce((sum,row)=>sum+n(row.count),0),
      open_actions:actions.rows.reduce((sum,row)=>sum+n(row.count),0)
    },
...
export function choosePrimary(population,operations,attention,attendanceLens) {
  const activePeople = n(population.people);
  const operational = n(operations.pending_scan_reviews)+n(operations.unhealthy_events)+n(operations.high_priority_operational_actions);
  const actionLoad = n(operations.open_actions||0);
  const observationLoad = n(operations.active_observations||0);
  const signalSaturation = activePeople>0 && (actionLoad>activePeople*1.5 || observationLoad>activePeople*2);
  ...
  if(signalSaturation) {
    return {
      key:'protect_signal_quality',
      title:'Protect ARIA’s signal quality before widening outreach',
      summary:'ARIA is carrying more active observations or actions than the current population reasonably needs for human review. I would narrow the signal field before asking people to act on all of it.',
      reason:'Too many simultaneous recommendations make meaningful care harder to see and can turn background history into apparent urgency.',
      priority:'high'
    };
  }
...
function buildInsight(core,attention,primary,contradictions) {
  ...
  const operational=n(core.operations.pending_scan_reviews)+n(core.operations.unhealthy_events)+n(core.operations.high_priority_operational_actions);
  const actionLoad=n(core.operations.open_actions);
  const observationLoad=n(core.operations.active_observations);
  if(people>0 && (actionLoad>people*1.5 || observationLoad>people*2)) {
    return {
      kind:'signal_saturation',
      title:'ARIA sees more signals than a human should have to carry at once',
      statement:'There are '+String(actionLoad)+' open actions and '+String(observationLoad)+' active observations across '+String(people)+' people. That does not mean all of them deserve human attention; ARIA should keep background memory separate from decision-worthy signals and keep the human queue intentionally small.',
      evidence:[
        evidenceLine(String(actionLoad)+' open proposed/approved/executing actions.','aria_actions',1),
        evidenceLine(String(observationLoad)+' active non-expired observations.','aria_observations',1),
        evidenceLine(String(people)+' active people.','people',1)
      ]
    };
  }
  ...
  if(people>=10 && days!==null && days<=14 && growth>=5) {
    return {
      ...
      title:'The organization is growing faster than its human context',
      statement:String(people)+' people are now known, but the population has only existed in the system for about '+String(days)+' day'+(days===1?'':'s')+' and '+String(growth)+' people arrived in the last seven days. Only '+String(coverage)+'% of people currently have explicit human context. ARIA already has system history such as attendance and observations; the next intelligence challenge is learning the human context around that history without turning people into statistics.',
      evidence:[
  ...
  if(people>0 && n(core.knowledge.org_memory)===0 && n(core.knowledge.human_feedback)===0 && n(core.knowledge.outcomes)===0) {
    return {
      kind:'learning_gap',
      title:'ARIA is still learning what is unique about this organization',
      statement:'I can reason from the organization’s lived data, but I do not yet have verified organization-specific norms or human outcome feedback. I should therefore stay especially evidence-led and learn from the first real corrections and outcomes rather than pretending generic care rules are local truth.',
      evidence:[
        evidenceLine('No current organization-specific memory rules are recorded.','organization_memory',1),
        evidenceLine('No care feedback or recorded care outcomes are available yet.','care_feedback + intelligence_outcomes',1)
      ]
    };
  }
...
export async function getDirectorBriefing(organizationId,{limit=8}={}) {
  ...
  const cacheKey=organizationId+':'+safeLimit;
  const cached=directorCache.get(cacheKey);
  if(cached && cached.expiresAt>Date.now()) return cached.value;
  if(directorInflight.has(cacheKey)) return directorInflight.get(cacheKey);
  const compute=(async()=>{
  ...
  const allCareSignals=attention.filter(x=>x.class==='human_care');
  const allOpportunities=attention.filter(x=>x.class==='organization_opportunity');
  const careSignals=allCareSignals.slice(0,safeLimit);
  ...
  const operationalSignals={
    ...
    high_priority_operational_actions:n(core.operations.high_priority_operational_actions)
  };
  ...
  const result={
    ...
    state:{
      ...
      sessions_today:n(core.sessions.sessions_today),
      sessions_30_days:n(core.sessions.sessions_30d),
      sessions_last_7_days:n(core.sessions.sessions_7d),
      active_attendees_30_days:n(core.population.active_attendees_30d),
    ...
    intelligence:{
      ...
      signal_load:{
        active_observations:n(core.operations.active_observations),
        meaningful_observations:n(core.operations.meaningful_observations),
        background_observations:Math.max(0,n(core.operations.active_observations)-n(core.operations.meaningful_observations)),
        open_actions:n(core.operations.open_actions),
        actions_per_person:n(core.population.people)?Number((n(core.operations.open_actions)/n(core.population.people)).toFixed(2)):0,
        observations_per_person:n(core.population.people)?Number((n(core.operations.active_observations)/n(core.population.people)).toFixed(2)):0
      },
      attention:{
        human_care_count:allCareSignals.length,
    ...
    decisions:{
      ...
      director_position:'ARIA is the synchronized decision layer for this organization. These priorities are reconciled from current evidence before they are narrated or exposed as actions.',
      ...
      what_not_to_do:[
        ...
        'Do not create a human action simply because a record exists. ARIA should earn attention through a meaningful change, opportunity, conflict or operational risk.',
        ...
        'Do not confuse generic care knowledge with this organization’s learned norms.',
        'Do not present missing human context as evidence that nothing is known.'
      ],
      learning_focus:(n(core.knowledge.org_memory)===0&&n(core.knowledge.human_feedback)===0&&n(core.knowledge.outcomes)===0)
        ?'ARIA has not yet learned an organization-specific norm from human feedback or outcomes. The first meaningful corrections, preferences and care outcomes should become durable organizational learning.'
        :insight.kind==='learning_gap'?insight.title:'Keep collecting human corrections and outcomes so future recommendations become organization-specific.'
    },
    evidence:{
      primary:[
    ...
    status:{
      trusted:operationalSignals.dead_events===0 && operationalSignals.unhealthy_events===0,
      data_freshness:'live',
      requires_human_decision:careSignals.some(x=>x.action_status==='proposed'),
      operational_risk:operationalSignals.failed_sessions>0 || operationalSignals.pending_scan_reviews>0 || operationalSignals.unhealthy_events>0 || operationalSignals.high_priority_operational_actions>0,
      knowledge_gaps:{
        organization_norms:n(core.knowledge.org_memory)===0,
        human_context_coverage:memoryCoverage<50,
        care_outcomes:n(core.knowledge.outcomes)===0,
        human_feedback:n(core.knowledge.human_feedback)===0
      }
    }
  };
  directorCache.set(cacheKey,{value:result,expiresAt:Date.now()+DIRECTOR_CACHE_TTL_MS});
  return result;
  })();
  directorInflight.set(cacheKey,compute);
  try{return await compute;}finally{directorInflight.delete(cacheKey);}
}
