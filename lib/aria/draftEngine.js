...
const attribution=n=>{const x=clean(n,160);return x?`From: ${x}`:'From: your church';};
const withAttribution=(message,name)=>{const body=clean(message,1500).replace(/\s*\n?\s*From:\s*.+$/i,'').trim();return `${body}\n\n${attribution(name)}`.slice(0,1600);};
...
export async function createCareDraft({organizationId,personId,actionId=null,actionType='thoughtful_check_in',actorId=null}){
 ...
 const person=p.rows[0],org=o.rows[0]||{},organizationName=clean(org.name,160);
