...
async function applyMemory(event,db){
 ...
 if(event.person_id){
  ...
  await setPersonMemory({
   ...
   sourceEventId:eventId,createdBy:event.actor_id||null,actorRole:event.actor_role||null,
  ...
  await setOrganizationMemory({
   ...
   source:event.source||'human',sourceEventId:eventId,createdBy:event.actor_id||null,
...
async function processClaimed(event,db){
 ...
 if(map){
  observationId=await createObservation({
   ...
   sourceEventId:eventId,expiresAt:event.expires_at||null
  },db);
 }
