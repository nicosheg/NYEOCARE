...
const OBSERVATION_MAP={
 ...
 PERSON_NOT_OBSERVED:{observationType:'PERSON_NOT_OBSERVED',severity:'low',urgency:'low',confidence:1,fact:'The person was not recorded as present for this event.',inference:'This is an observation only; the reason for non-observation is unknown.'},
...
const MEMORY_EVENTS=new Set(['ADMIN_CORRECTION','NEW_CONTEXT_ADDED','LEADER_NOTE_ADDED','COMMUNICATION_PREFERENCE_CHANGED','ORGANIZATION_RULE_LEARNED']);
...
async function learnEvent(event){
 ...
 await recordLearning({
  ...
  learningDomain:'personalized',learningContext:'communication',privacyClass:'identity_private'
 });
}
...
async function processClaimed(event,db){
 ...
 if(LEARNING_EVENTS.has(event.type))await learnEvent(event);
 if(event.person_id&&['PERSON_CREATED','PERSON_UPDATED','PARTICIPATION_CONFIRMED','PERSON_NOT_OBSERVED','RETURNED_AFTER_ABSENCE','CARE_FEEDBACK','ADMIN_CORRECTION','NEW_CONTEXT_ADDED'].includes(event.type)){
