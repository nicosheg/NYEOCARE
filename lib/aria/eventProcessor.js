...
async function resolveSupersededAttention(event,db){
 if(!['PARTICIPATION_CONFIRMED','RETURNED_AFTER_ABSENCE'].includes(event.type))return;
 if(event.type==='PARTICIPATION_CONFIRMED' && event.metadata?.session_id){
  await db.query(`UPDATE aria_observations SET status='resolved',resolved_at=NOW(),updated_at=NOW() WHERE organization_id=$1 AND type='ARIA_PROCESSING_FAILURE' AND status='active' AND metadata->>'session_id'=$2`,[event.organization_id,String(event.metadata.session_id)]);
  await db.query(`UPDATE aria_actions SET status='cancelled',failure_reason='Superseded by successful attendance processing.',updated_at=NOW() WHERE organization_id=$1 AND status='proposed' AND action_metadata->>'kind'='aria_processing_failure' AND action_metadata->>'session_id'=$2`,[event.organization_id,String(event.metadata.session_id)]);
 }
 ...
 const observations=await db.query(`
  ...
  WHERE o.organization_id=$1 AND o.person_id=$2 AND o.type='UNUSUAL_ABSENCE' AND o.status='active'
    AND o.detected_at<$3
  RETURNING id
 `,[event.organization_id,event.person_id,occurred]);
 if(observations.rows.length){
  await db.query(`
   UPDATE aria_actions a
   SET status='cancelled',failure_reason='Superseded by later confirmed participation or return.',updated_at=NOW()
   WHERE a.organization_id=$1 AND a.person_id=$2 AND a.status='proposed'
     AND a.action_metadata->>'kind' IN('attendance_absence_check_in','returned_after_absence')
     AND(a.proposed_at<$3 OR a.proposed_at IS NULL)
  `,[event.organization_id,event.person_id,occurred]);
 }
}
...
async function processClaimed(event,db){
 ...
 await resolveSupersededAttention(event,db);
