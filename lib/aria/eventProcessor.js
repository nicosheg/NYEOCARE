...
export async function processAriaEvent(event,client=null){
 ...
 try{
  ...
  if(current.person_id)await updatePeopleIntelligence(current.person_id,current.organization_id,db);
  if(current.person_id)await updatePersonState(current.person_id,current.organization_id,db);
  ...
  if(!client){}
  try{await db.query(`UPDATE aria_events SET processing_status='failed',last_processing_error=$3 WHERE id=$1 AND organization_id=$2`,[event.id,event.organization_id,clean(err?.message||String(err),1000)])}catch{}
