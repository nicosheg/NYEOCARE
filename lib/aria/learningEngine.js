...
export async function recordLearning({
 ...
 const result=await db.query(
  'INSERT INTO aria_learning(organization_id,person_id,scope_key,learning_type,learning_key,value,confidence,source_type,source_id,active,learning_domain,scope_level,learning_context,privacy_class,evidence_kind,verification_status,valid_until,actor_id,created_at,updated_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9,true,$10,$11,$12,$13,$14,$15,$16,$17,NOW(),NOW()) ON CONFLICT(organization_id,scope_key,learning_type,learning_key) DO UPDATE SET value=EXCLUDED.value,confidence=GREATEST(aria_learning.confidence,EXCLUDED.confidence),source_type=EXCLUDED.source_type,source_id=EXCLUDED.source_id,active=true,learning_domain=EXCLUDED.learning_domain,scope_level=EXCLUDED.scope_level,learning_context=EXCLUDED.learning_context,privacy_class=EXCLUDED.privacy_class,evidence_kind=EXCLUDED.evidence_kind,verification_status=EXCLUDED.verification_status,valid_until=EXCLUDED.valid_until,actor_id=EXCLUDED.actor_id,updated_at=NOW() RETURNING *',
  [organizationId,personId,key,learningType,learningKey,safeValue,safeConfidence,sourceType,sourceId,learningDomain,level,learningContext,privacyClass,evidenceKind,verificationStatus,validUntil,actorId||userId||null]
 );
...
export async function recordGlobalLearning({learningType,learningKey,value={},confidence=.5,sourceType='human_review',sourceId=null},client=null){
 ...
 const db=client||pool;
 const result=await db.query(
  'INSERT INTO aria_global_learning(learning_type,learning_key,value,confidence,example_count,source_type,source_id,active,created_at,updated_at) VALUES($1,$2,$3,$4,1,$5,$6,true,NOW(),NOW()) ON CONFLICT(learning_type,learning_key) DO UPDATE SET value=EXCLUDED.value,confidence=GREATEST(aria_global_learning.confidence,EXCLUDED.confidence),example_count=aria_global_learning.example_count+1,source_type=EXCLUDED.source_type,source_id=EXCLUDED.source_id,active=true,updated_at=NOW() RETURNING *',
  [learningType,learningKey,value&&typeof value==='object'?value:{},safeConfidence,sourceType,sourceId]
 );
 return result.rows[0];
}
...
export async function learnFromOutcome({organizationId,personId,actionId,outcome,outcomeScore,evidence={}},client=null){
 ...
 return recordLearning({organizationId,personId:null,learningType:'action_outcome',learningKey:String(outcome),value:{action_id:actionId,person_id:personId,outcome,outcome_score:score,evidence},confidence:clamp(confidence,0,1),sourceType:'outcome',sourceId:actionId,scopeLevel:'organization',scopeKey:'organization',learningDomain:'personalized',learningContext:'personalized',privacyClass:'organization_private'},client);
}
...
export async function getLearnings(organizationId,{personId=null,userId=null,learningType=null,learningDomain=null,limit=50}={}){
 ...
 const safeLimit=Math.min(Math.max(Number(limit)||50,1),200),params=[organizationId];let where='organization_id=$1 AND active=true';
 ...
 return(await pool.query('SELECT * FROM aria_learning WHERE '+where+' AND(valid_until IS NULL OR valid_until>NOW()) ORDER BY confidence DESC,updated_at DESC LIMIT 
}
