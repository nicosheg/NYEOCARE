...
export async function setPersonMemory({
 ...
 const conflicts=Boolean(current.rows[0]&&current.rows[0].content!==incoming&&((evidenceKind==='human_report'||verificationStatus==='reported')||(current.rows[0].evidence_kind==='human_report'||current.rows[0].verification_status==='reported')));
 if(current.rows[0])await db.query('UPDATE person_memory SET is_current=false,active=false,verification_status=CASE WHEN $2 THEN \'conflicted\' ELSE verification_status END,valid_until=COALESCE($3,NOW()),updated_at=NOW(),metadata=CASE WHEN $2 THEN COALESCE(metadata,\'{}\'::jsonb)||jsonb_build_object(\'conflict_with_id\',$1::text,\'conflict_detected_at\',NOW()) ELSE metadata END WHERE id=$1',[current.rows[0].id,conflicts,validFrom||null]);
 const result=await db.query('INSERT INTO person_memory(organization_id,person_id,memory_type,memory_key,content,importance,confidence,source_event_id,source,active,metadata,is_current,evidence_kind,verification_status,valid_from,valid_until,created_by,actor_role,verified_at,supersedes_id) VALUES($1,$2,$3,$4,$5,$6,$7,$8,true,$9,true,$10,$11,COALESCE($12,NOW()),$13,$14,$15,$16,$17) RETURNING *',[organizationId,personId,memoryType,memoryKey,incoming,importance==='permanent'?'permanent':importance==='important'?'important':'temporary',clamp(confidence),sourceEventId||null,source,obj({...metadata,...(conflicts?{conflict_with_id:current.rows[0].id,conflict_detected_at:new Date().toISOString()}: {})}),evidenceKind,conflicts?'conflicted':verificationStatus,validFrom||null,validUntil||null,createdBy||null,actorRole||null,verificationStatus==='verified'&&!conflicts?new Date().toISOString():null,current.rows[0]?.id||null]);
...
export async function setOrganizationMemory({
 ...
 const orgConflicts=Boolean(current.rows[0]&&JSON.stringify(current.rows[0].memory_value)!==JSON.stringify(next)&&((evidenceKind==='human_report'||verificationStatus==='reported')||(current.rows[0].evidence_kind==='human_report'||current.rows[0].verification_status==='reported')));
 if(current.rows[0])await db.query('UPDATE organization_memory SET is_current=false,verification_status=CASE WHEN $2 THEN \'conflicted\' ELSE verification_status END,valid_until=COALESCE($3,NOW()),updated_at=NOW(),metadata=CASE WHEN $2 THEN COALESCE(metadata,\'{}\'::jsonb)||jsonb_build_object(\'conflict_with_id\',$1::text,\'conflict_detected_at\',NOW()) ELSE metadata END WHERE id=$1',[current.rows[0].id,orgConflicts,validFrom||null]);
 const result=await db.query('INSERT INTO organization_memory(organization_id,memory_type,memory_key,memory_value,confidence,source,created_at,updated_at,is_current,evidence_kind,verification_status,valid_from,valid_until,source_event_id,created_by,verified_at,supersedes_id,metadata) VALUES($1,$2,$3,$4,$5,$6,NOW(),NOW(),true,$7,$8,COALESCE($9,NOW()),$10,$11,$12,$13,$14,$15) RETURNING *',[organizationId,memoryType,memoryKey,next,clamp(confidence),source,evidenceKind,orgConflicts?'conflicted':verificationStatus,validFrom||null,validUntil||null,sourceEventId||null,createdBy||null,verificationStatus==='verified'&&!orgConflicts?new Date().toISOString():null,current.rows[0]?.id||null,obj({...metadata,...(orgConflicts?{conflict_with_id:current.rows[0].id,conflict_detected_at:new Date().toISOString()}: {})})]);
 return result.rows[0];
}
