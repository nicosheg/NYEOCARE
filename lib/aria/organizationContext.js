...
export async function getOrganizationContext({organizationId,viewerId}){
 ...
 const [org,peopleCounts,people,birthdays,tasks,intelligence,operators,sessions,scans,invitations,memory,groups]=await Promise.all([
  ...
  pool.query("SELECT id,name,status,service_type,started_at,closed_at,event_kind,event_scope,group_id,event_semantics,expected_population_rule,attendance_interpretation,participation_expected,optional,absence_meaningful,aria_processing_status FROM sessions WHERE organization_id=$1 ORDER BY started_at DESC NULLS LAST LIMIT 20",[organizationId]),
  ...
  getInvitations(organizationId,viewer.role),getMemory(organizationId),pool.query("SELECT g.id,g.name,g.group_type,g.description,g.active,g.metadata,COUNT(DISTINCT CASE WHEN pm.status='active' THEN pm.person_id END)::int AS member_count FROM organization_groups g LEFT JOIN person_memberships pm ON pm.organization_id=g.organization_id AND pm.group_id=g.id WHERE g.organization_id=$1 AND g.active=true GROUP BY g.id ORDER BY g.name LIMIT 100",[organizationId])
 ]);
