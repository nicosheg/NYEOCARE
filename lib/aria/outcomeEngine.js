...
export async function recordOutcome(orgId,personId,outcomeType,outcomeScore=null,actionId=null,evidence={},actorId=null,feedbackId=null,sourceEventId=null){
 ...
 try{
  const event=await emitAriaEvent({organizationId:orgId,personId,type:'CARE_OUTCOME_RECORDED',source:'outcome',actorId,evidenceKind:'observation',verificationStatus:'observed',confidence:outcomeScore===null?.8:1,metadata:{outcome_id:outcome.id,action_id:actionId,outcome:outcomeType,outcome_score:outcomeScore,evidence:evidence&&typeof evidence==='object'?evidence:{},statement:'Care outcome recorded.'},eventKey:'outcome:'+outcome.id});
  if(event)processAriaEvent(event).catch(error=>console.warn('[ARIA] outcome event processing deferred:',error?.message||error));
 }catch(error){console.warn('[ARIA] outcome event emission deferred:',error?.message||error)}
