...
// The DB pool is capped at 4 connections per warm instance. Four workers
// keep the attendance intelligence pass fast without exceeding that ceiling.
const CONCURRENCY=4;
 ...
 if(participationIds.length){
  try{
   const created=await pool.query(`INSERT INTO aria_events(organization_id,person_id,type,actor_id,source,event_key,metadata,occurred_at)
    SELECT $1,p.person_id,'PARTICIPATION_CONFIRMED',NULL,'attendance','participation:'||p.participation_id||':confirmed',jsonb_build_object('session_id',$3::uuid,'participation_id',p.participation_id),NOW()
