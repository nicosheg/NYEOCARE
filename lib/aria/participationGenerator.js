...
export async function generateParticipationFromSession(sessionId,orgId){
 ...
 try{
  ...
  const session=await client.query(
   `SELECT id FROM sessions WHERE id=$1 AND organization_id=$2 LIMIT 1`,
   [sessionId,orgId]
  );
  ...
  const attendance=await client.query(
   `SELECT ar.id,ar.people_id,ar.attendance_date
  ...
  for(const row of attendance.rows){
   ...
   records.push({
    participationId:inserted.rows[0]?.id||null,
 ...
 for(const row of records){
  ...
  try{
   const event=await emitAriaEvent({
    ...
    eventKey:`participation:${row.participationId}:confirmed`
   });

   if(event)await processAriaEvent(event);
  }catch(error){
   console.error('[ARIA] participation event:',error.message);
  }
 }

 for(const personId of personIds){
  try{await updateEngagementMetricsForPerson(personId,orgId)}
  catch(error){console.error('[ARIA] engagement:',error.message)}
 }

 try{await computeRelationshipScore(orgId)}
 catch(error){console.error('[ARIA] relationship:',error.message)}
 ...
 for(const personId of personIds){
  try{
   ...
   console.error('[ARIA] intelligence:',error.message);
  }
 }

 try{await generateCareRecommendations(orgId)}
 catch(error){console.error('[ARIA] care:',error.message)}

 return{
  session_id:sessionId,
  processed:personIds.length,
  events:records.filter(x=>x.participationId).length
 };
    }
