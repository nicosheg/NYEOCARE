...
export async function updatePeopleIntelligence(personId,orgId,client=null){
 ...
 const[r,m,o,f,mem,learn,rel]=await Promise.all([
  ...
  db.query(`SELECT score FROM relationship_scores WHERE organization_id=$1 AND person_id=$2 LIMIT 1`,[orgId,personId])
 ]);
 if(!r.rows.length)throw new Error('Person not found');
 const x=m.rows[0]||{},obs=o.rows[0]||{},fb=f.rows[0]||{},memoryCount=Number(mem.rows[0]?.count)||0,learning=learn.rows[0]?.learning_key||null,relationship=Math.max(0,Number(rel.rows[0]?.score)||0);
