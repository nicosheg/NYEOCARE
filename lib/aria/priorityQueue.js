...
// Only observations that can reasonably change a human decision belong in the attention queue.
const ATTENTION_OBSERVATION_TYPES=['UNUSUAL_ABSENCE','SCAN_REVIEW_REQUIRED','ARIA_PROCESSING_FAILURE','CONFLICT','DATA_INTEGRITY','SECURITY_REVIEW'];
  ...
  WITH base AS(
   ...
   LEFT JOIN(
    ...
    WHERE status='active'
      ...
      AND type IN('UNUSUAL_ABSENCE','SCAN_REVIEW_REQUIRED','ARIA_PROCESSING_FAILURE','CONFLICT','DATA_INTEGRITY','SECURITY_REVIEW')
    GROUP BY organization_id,person_id
