import pool from'../db';
import{clampConfidence,evidence,detectConflicts}from'./epistemic';
...
async function build({organizationId,personId,client=null}){
 ...
 const[person,participation,observations,feedback,memory,actions]=await Promise.all([
  db.query("SELECT id,display_name,first_name,last_name,type,source,metadata,identity_verification_status,identity_verified_at FROM people WHERE id=$1 AND organization_id=$2 AND COALESCE(status,'active')='active' LIMIT 1",[personId,organizationId]),
 ...
 const p=person.rows[0],identityStatus=String(p.identity_verification_status||'unverified'),identityTrusted=identityStatus==='verified'||String(p.source||'')==='manual'||p.metadata?.identity_verified===true;
 const claims=[{key:'identity.status',value:identityStatus,status:identityStatus==='conflict'?'conflicted':identityTrusted?'verified':'observed',source:'people.identity_verification',source_id:p.id,occurred_at:p.identity_verified_at||null,confidence:identityTrusted?1:.7}];
 ...
 const conflicts=identityStatus==='conflict'?detectConflicts([{key:'identity.status',value:'conflict',status:'conflicted',source:'people.identity_verification',source_id:p.id,occurred_at:p.identity_verified_at||null},{key:'identity.status',value:'non_conflict',status:'verified',source:'human_review',source_id:p.id,occurred_at:p.identity_verified_at||null}]):[];
 const unknowns=[];
