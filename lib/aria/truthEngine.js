...
async function build({organizationId,personId,client=null}){
 ...
 const[person,participation,observations,feedback,memory,actions]=await Promise.all([
  ...
  db.query(`SELECT id,feedback_type,sentiment,note,observed_at,evidence_kind,verification_status,confidence,expires_at FROM care_feedback WHERE organization_id=$1 AND person_id=$2 AND(expires_at IS NULL OR expires_at>NOW()) ORDER BY observed_at DESC LIMIT 8`,[organizationId,personId]),
  db.query(`SELECT id,memory_type,memory_key,content,importance,confidence,source,source_event_id,metadata,evidence_kind,verification_status,valid_from,valid_until,created_by,actor_role,verified_at,updated_at FROM person_memory WHERE organization_id=$1 AND person_id=$2 AND active=true AND is_current=true AND(valid_until IS NULL OR valid_until>NOW()) ORDER BY CASE importance WHEN'permanent'then 4 WHEN'important'then 3 WHEN'temporary'then 2 ELSE 1 END DESC,confidence DESC,updated_at DESC LIMIT 20`,[organizationId,personId]),
 ...
 const p=person.rows[0],identityStatus=String(p.identity_verification_status||'unverified'),storedTruth=p.living_truth&&typeof p.living_truth==='object'?p.living_truth:{},storedStatus=String(storedTruth.status||'');
 const identityTrusted=identityStatus==='verified'||['manual','human_review'].includes(String(p.source||''))||p.metadata?.identity_verified===true;
 ...
 const activeObservations=observations.rows.map(o=>evidence({kind:'observation',status:o.status==='resolved'?'stale':'observed',source:'aria_observation',sourceId:o.id,statement:text(o.evidence?.summary||o.evidence?.inference||o.metadata?.summary||o.type,600),occurredAt:o.detected_at,confidence:clampConfidence(o.confidence,.6),metadata:{type:o.type,severity:o.severity,urgency:o.urgency,expires_at:o.expires_at}}));
 const humanReports=feedback.rows.map(f=>evidence({kind:f.evidence_kind==='fact'?'fact':'human_report',status:f.verification_status||'reported',source:'care_feedback',sourceId:f.id,statement:text(f.note||('Human feedback: '+f.feedback_type),600),occurredAt:f.observed_at,confidence:clampConfidence(f.confidence,.8),metadata:{sentiment:f.sentiment,expires_at:f.expires_at}}));
...
export async function refreshLivingTruth({organizationId,personId}={},client=null){
 if(!organizationId||!personId)throw new Error('organizationId and personId are required');
 const db=client||pool,truth=await build({organizationId,personId,client:db});
