...
const NAME_WEIGHT=.65,PHONE_WEIGHT=.35,CANDIDATE_THRESHOLD=60,AUTO_THRESHOLD=93,CONFLICT_MARGIN=6;
...
function duplicateScore(name,phone){return Math.round(name*NAME_WEIGHT+phone.score*PHONE_WEIGHT)}
function method(name,phone){if(phone.count&&name>=85)return phone.count>1?'name_and_two_full_phones':'name_and_full_phone';if(phone.count)return'full_phone_only';if(phone.suffix_match>=4)return'last_4_digits_plus_name';if(phone.suffix_match>=2)return'last_2_digits_plus_name';if(phone.suffix_match>=1)return'last_1_digit_plus_name';if(name>=96)return'exact_name_only';return'fuzzy_name_only'}
function candidateEvidence(person,extractedName,extractedPhones){const canonical=displayName(person),name=nameScore(extractedName,canonical),phone=phoneEvidence(extractedPhones,personPhones(person));return{...person,score:duplicateScore(name,phone),nameScore:name,phoneScore:phone.score,phoneMatches:phone.matches,phoneMatchCount:phone.count,suffixMatch:phone.suffix_match,phoneRelation:phone.relation,suffixMatches:phone.suffix_matches,compatibleName:name>=60,method:method(name,phone)}}
export async function resolveIdentities(extractedPeople,orgId,scanJobId,client=null){
 ...
 for(let idx=0;idx<extractedPeople.length;idx++){
  ...
  for(const row of people){const evidence=candidateEvidence(row,extractedName,normalizedPhones);if(evidence.nameScore>=60&&(!normalizedPhones.length||evidence.phoneScore>0))candidates.set(row.id,evidence)}
  for(const id of[...new Set(aliasMap.get(key)||[])]){const row=byId.get(id);if(row)candidates.set(id,candidateEvidence(row,extractedName,normalizedPhones))}
  const exactPhoneCandidates=normalizedPhones.flatMap(p=>phoneMap.get(p)||[]);for(const row of exactPhoneCandidates)candidates.set(row.id,candidateEvidence(row,extractedName,normalizedPhones));
  const unique=[...candidates.values()].sort((a,b)=>b.score-a.score),best=unique[0]||null,second=unique[1]||null,margin=best&&second?best.score-second.score:100,exactOwners=[...new Map(exactPhoneCandidates.map(row=>[row.id,row])).values()];
  let status='new',bestCandidateId=null;
  if(best&&best.nameScore>=60&&best.score>=CANDIDATE_THRESHOLD){if(second&&best.score>=CANDIDATE_THRESHOLD&&second.score>=CANDIDATE_THRESHOLD&&margin<=CONFLICT_MARGIN)status='conflict';else if(best.score>=AUTO_THRESHOLD&&best.nameScore>=82&&best.phoneScore>=65&&(!second||margin>CONFLICT_MARGIN)){status='alive';bestCandidateId=best.id}else status='needs_decision'}
