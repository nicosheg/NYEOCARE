...
const initialState={stage:'idle',jobId:null,scanningLine:false,results:null,revealedPeople:[],ariaMessages:[],summary:null,message:'',progress:null};
function read(){if(typeof window==='undefined')return initialState;try{const raw=window.localStorage.getItem(KEY);if(!raw)return initialState;const parsed=JSON.parse(raw);return parsed&&typeof parsed==='object'?{...initialState,...parsed}:initialState}catch{return initialState}}
let scanState=read();
...
export function setScanState(newState){scanState={...scanState,...newState};notify();return getScanState()}
export function clearScanState(force=false){if(scanState.stage==='processing'&&!force)return getScanState();scanState={...initialState};notify();return getScanState()}
