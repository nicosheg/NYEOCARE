...
extracted_name:rawName||null,
extracted_phone:phone,
confidence:Math.min(confidence,60),
row_number:row,
reasons:[...reasons,'name_needs_verification'],
verification_alternatives:raw.verification_alternatives||null,
incoming:{name:rawName||null,phone,rawPhone,row_number:row}
});
continue;
}

if(verificationStatus!=='verified'||!phoneCheck.valid||confidence<98){
...
needsReview.push({
status:'needs_decision',
extracted_name:cleanDisplay||rawName,
extracted_phone:phone,
confidence,
row_number:row,
reasons:[...new Set(reviewReasons)],
verification_alternatives:raw.verification_alternatives||null,
incoming:{name:cleanDisplay||rawName,phone,rawPhone,row_number:row}
});
continue;
}

if(seenPhones.has(phone)||seenNames.has(nameKey)){
rejected.push({name:cleanDisplay||rawName,phone,reason:'Duplicate row in same scan',row_number:row});
continue;
}
