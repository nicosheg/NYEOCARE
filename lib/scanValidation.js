...
export async function validateScanOutput(rawContent,orgId,programName,jobId,options={}){
 ...
 for(const raw of parsed.people){
  ...
  const rawName=String(raw.name||[raw.first_name,raw.last_name].filter(Boolean).join(' ')).trim();
  ...
  if(!nameKey){
   rejected.push({name:rawName,phone:rawPhone,reason:'Name could not be safely read'});
   continue;
  }
  ...
  const confidence=normalizeConfidence(Number.isFinite(Number(raw.confidence))?Number(raw.confidence):(phone?70:45),phone?70:45);
