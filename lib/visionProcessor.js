...
function reviewFor(person,validation){
 return validation.needsReview.find(x=>x.row_number===person.row_number)||(validation.needsReview.find(x=>x.extracted_phone&&x.extracted_phone===person.phone))||(validation.needsReview.find(x=>x.extracted_name&&x.extracted_name===person.name))||null;
}
async function insertQuarantine(client,orgId,jobId,actorId,programName,person,review){
 const reason=review?.reason||'ARIA found something that needs your attention.';
 const metadata={scan_pipeline_version:PIPELINE_VERSION,honorific:person.honorific||null,row_number:person.row_number,scan_evidence_job_id:jobId,vision_provider:'groq',phone_relation:person.phone_relation||null,link_evidence:person.link_evidence||null,raw_name:person.raw_name||null,raw_phone:person.rawPhone||null,confidence:person.confidence,review_reasons:review?.reasons||[],review_suggestion:review?.suggestion||null,program_name:programName};
 const result=await client.query(`INSERT INTO people(organization_id,first_name,last_name,display_name,phone,type,status,confidence,source,created_by,last_scan_job_id,quarantine_reason,quarantined_at,living_truth,metadata) VALUES($1,$2,$3,$4,$5,'visitor','quarantined',$6,'scan',$7,$8,$9,NOW(),$10,$11) RETURNING id,display_name,phone`,[orgId,person.first_name,person.last_name,person.display_name,person.phone,normalizeConfidence(person.confidence,70),actorId,jobId,reason,JSON.stringify({status:'quarantined',source:'scan',updated_at:new Date().toISOString()}),JSON.stringify(metadata)]);
...
export async function processVisionJob(jobId,imageBase64,orgId,programName,options={}){
 ...
 try{
  ...
  let vision;
  try{
   vision=await callVisionWithRetry(imageBase64,null,progress=>{if(!evaluation)updateJobState(client,jobId,INTERNAL_STATES.EXTRACTING,progress,null,vision?.attempt||1,'groq',startTime).catch(()=>{})},{organization_id:orgId,job_id:jobId,purpose:'scan',prompt_version:PIPELINE_VERSION,evaluation});
  }catch(err){
   const e=errorResult('ai_request',err.code||'AI_PROVIDER_ERROR',err.message||'Vision verification failed.','ARIA could not safely read this register. Nothing was changed.');
   if(!evaluation)try{await updateJobState(client,jobId,INTERNAL_STATES.FAILED,'failed',e,0,'groq',startTime)}catch{}
   return e;
  ...
  if(typeof rawContent!=='string'||!rawContent.trim()){
   ...
   if(!evaluation)try{await updateJobState(client,jobId,INTERNAL_STATES.FAILED,'failed',e,vision?.attempt||0,'groq',startTime)}catch{}
   return e;
  }
  ...
  if(!validation.valid){
   const e=errorResult('parse_json','INVALID_AI_RESPONSE',validation.error,'ARIA could not safely understand the register.');
   if(!evaluation)try{await updateJobState(client,jobId,INTERNAL_STATES.FAILED,'failed',e,vision.attempt,'groq',startTime)}catch{}
   return e;
  }
  ...
  for(const person of validation.people){
   ...
   if(review){
    ...
    quarantinedPeople.push({id:candidate.id,name:candidate.display_name,phone:candidate.phone,reason:review.reason,suggestion:review.suggestion,row_number:person.row_number,confidence:person.confidence});
    continue;
   }
   ...
   const first=person.first_name,last=person.last_name,stage=person.relationship_stage,type=['regular','returning','familiar_face'].includes(stage)?'member':'visitor',confidence=normalizeConfidence(person.confidence,90);
   const metadata={scan_pipeline_version:PIPELINE_VERSION,honorific:person.honorific||null,row_number:person.row_number,scan_evidence_job_id:jobId,vision_provider:'groq',phone_relation:person.phone_relation||null,link_evidence:person.link_evidence||null,raw_name:person.raw_name||null,raw_phone:person.rawPhone||null};
   const insert=await client.query(`INSERT INTO people(organization_id,first_name,last_name,display_name,phone,type,status,confidence,source,created_by,last_scan_job_id,living_truth,metadata) VALUES($1,$2,$3,$4,$5,$6,'active',$7,'scan',$8,$9,$10,$11) RETURNING id,display_name,phone,type`,[orgId,first,last,person.display_name,person.phone,type,confidence,actorId,jobId,JSON.stringify({status:'alive',confidence,source:'scan',updated_at:new Date().toISOString()}),JSON.stringify(metadata)]);
   const created=insert.rows[0];
   newPeople.push({id:created.id,name:created.display_name,type:created.type});
   const event=await emitAriaEvent({organizationId:orgId,personId:created.id,type:'PERSON_CREATED',source:'scan',actorId,metadata:{jobId,programName,confidence,pipeline_version:PIPELINE_VERSION},eventKey:`scan:${jobId}:person:${created.id}`},client);
   if(event)events.push(event);
  }
  const result={status:'ok',scan_pipeline_version:PIPELINE_VERSION,new_members:newPeople.length,updated:matchedPeople.length,duplicates:[],needs_review:quarantinedPeople,rejected:validation.rejected,people:validation.people.map(p=>({name:p.display_name,phone:p.phone,confidence:p.confidence,row_number:p.row_number,review:Boolean(reviewFor(p,validation))})),total_extracted:validation.total_extracted,total_valid:validation.total_valid,register_mode:registerMode,verification:{accepted:validation.people.length-quarantinedPeople.length,review:quarantinedPeople.length,extraction_quality:vision.verification?.extraction_quality||0,passes:vision.verification?.passes||1},evidence:{available:true,job_id:jobId},layout:{provider:'groq-direct-vision',rows:validation.total_extracted},summary:`ARIA found ${validation.total_extracted} people, safely remembered ${newPeople.length+matchedPeople.length}, and sent ${quarantinedPeople.length} to Review Center.`};
  ...
  await finalizeJob(jobId,result,vision.attempt,startTime);
  return result;
 }catch(err){
