...
const PIPELINE_VERSION='v13-simple-name-phone';
...
async function finalizeJob(jobId,result,attempt,startTime){
 ...
 try{await updateJobState(c,jobId,INTERNAL_STATES.COMPLETED,'complete',result,attempt,'groq',startTime);return true}
 catch(err){console.error(`[SCAN] Final status update failed for ${jobId}:`,err?.message||err);return false}
 finally{c.release()}
}

function reviewFor(person,validation){
 return validation.needsReview.find(x=>x.extracted_name&&x.extracted_name===person.name)||(validation.needsReview.find(x=>x.extracted_phone&&x.extracted_phone===person.phone))||null;
}
...
async function insertQuarantine(client,orgId,jobId,actorId,programName,person,review){
 ...
 const phoneNumbers=phoneRecords(person);
 const metadata={scan_pipeline_version:PIPELINE_VERSION,honorific:person.honorific||null,row_number:person.row_number,scan_evidence_job_id:jobId,vision_provider:'groq',phone_numbers:phoneNumbers,raw_name:person.raw_name||null,raw_phones:person.rawPhones||[],model_confidence:person.model_confidence??null,name_confidence:person.name_confidence??null,phone_confidence:person.phone_confidence??null,confidence:person.confidence,review_reasons:review?.reasons||[],review_suggestion:review?.suggestion||null,program_name:programName};
...
export async function processVisionJob(jobId,imageBase64,orgId,programName,options={}){
 ...
 try{
  ...
  try{
   vision=await callVisionWithRetry(imageBase64,null,progress=>{
    if(!evaluation)updateJobState(client,jobId,INTERNAL_STATES.EXTRACTING,progress,null,vision?.attempt||1,'groq',startTime).catch(()=>{})
   ...
   await releaseGroqLock(client);groqLocked=false;
  }
  ...
  for(const person of validation.people){
   ...
   if(review){
    ...
    quarantinedPeople.push({id:candidate.id,name:candidate.display_name,phone:candidate.phone,phones:candidate.phone_numbers||[],reason:review.reason,suggestion:review.suggestion,row_number:person.row_number,confidence:person.confidence});
    continue;
   }
   ...
   const first=person.first_name,last=person.last_name,stage=person.relationship_stage,type=['regular','returning','familiar_face'].includes(stage)?'member':'visitor',confidence=normalizeConfidence(person.confidence,90),phoneNumbers=phoneRecords(person);
   const metadata={scan_pipeline_version:PIPELINE_VERSION,honorific:person.honorific||null,row_number:person.row_number,scan_evidence_job_id:jobId,vision_provider:'groq',phone_numbers:phoneNumbers,raw_name:person.raw_name||null,raw_phones:person.rawPhones||[],model_confidence:person.model_confidence??null,name_confidence:person.name_confidence??null,phone_confidence:person.phone_confidence??null};

   const insert=await client.query(`INSERT INTO people(organization_id,first_name,last_name,display_name,phone,phone_numbers,type,status,confidence,source,created_by,last_scan_job_id,living_truth,metadata) VALUES($1,$2,$3,$4,$5,$6,$7,'active',$8,'scan',$9,$10,$11,$12) RETURNING id,display_name,phone,phone_numbers,type`,[orgId,first,last,person.display_name,person.phone,JSON.stringify(phoneNumbers),type,confidence,actorId,jobId,JSON.stringify({status:'alive',confidence,source:'scan',updated_at:new Date().toISOString()}),JSON.stringify(metadata)]);
  ...
  const result={
   status:'ok',
   scan_pipeline_version:PIPELINE_VERSION,
   new_members:newPeople.length,
   updated:matchedPeople.length,
   duplicates:[],
   needs_review:quarantinedPeople,
   rejected:validation.rejected,
   people:validation.people.map(p=>({name:p.display_name,phone:p.phone,phones:p.phones||[],raw_phones:p.rawPhones||[],confidence:p.confidence,name_confidence:p.name_confidence,phone_confidence:p.phone_confidence,review:Boolean(reviewFor(p,validation))})),
   total_extracted:validation.total_extracted,
   total_valid:validation.total_valid,
   register_mode:registerMode,
   verification:{accepted:validation.people.length-quarantinedPeople.length,review:quarantinedPeople.length,extraction_quality:vision.verification?.extraction_quality||0,passes:vision.verification?.passes||1},
   evidence:{available:true,job_id:jobId},
   layout:{provider:'groq-direct-vision',rows:validation.total_extracted},
   summary:`ARIA found ${validation.total_extracted} people, safely remembered ${newPeople.length+matchedPeople.length}, and sent ${quarantinedPeople.length} to Review Center.`
  };
