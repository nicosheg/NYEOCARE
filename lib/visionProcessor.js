 ...
 try{
  ...
  if(!evaluation){
   ...
   if(cached.rows.length){
    ...
    await finalizeJob(jobId,cachedResult,0,startTime);
    return cachedResult;
   }
  }
  ...
  for(const decision of identity.decisions){
   ...
   const metadata={scan_pipeline_version:PIPELINE_VERSION,honorific:source?.honorific||null,row_number:source?.row_number,scan_evidence_job_id:jobId,vision_provider:'groq',phone_relation:source?.phone_relation||null,link_evidence:source?.link_evidence||null};
  ...
  const result={status:'ok',scan_pipeline_version:PIPELINE_VERSION,new_members:newPeople.length,updated:matchedPeople.length,duplicates:[],needs_review:combinedReview,rejected:validation.rejected,people:validation.people.map(p=>({name:p.display_name||p.name,phone:p.phone,confidence:p.confidence,row_number:p.row_number,phone_relation:p.phone_relation,link_evidence:p.link_evidence||null})),total_extracted:validation.total_extracted,total_valid:validation.total_valid,register_mode:registerMode,verification:{accepted:validation.people.length,review:combinedReview.length,passes:vision.verification?.passes||1},evidence:{available:true,job_id:jobId},layout:{provider:'groq-direct-vision',rows:validation.total_extracted},summary:`ARIA verified ${validation.people.length} entries and held ${combinedReview.length} entries for human review. Displaced handwriting and visible links were considered.`};
