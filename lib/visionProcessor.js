...
const PIPELINE_VERSION='v16-scan-review-flow';
...
async function insertPendingPerson(client,orgId,jobId,actorId,programName,person,review,candidates=[]){
 const phones=phoneRecords(person),reason=review?.reason||'ARIA found something that needs your attention.',now=new Date().toISOString();
 const metadata={scan_pipeline_version:PIPELINE_VERSION,honorific:person.honorific||null,row_number:person.row_number,scan_evidence_job_id:jobId,vision_provider:'groq',phone_numbers:phones,raw_name:person.raw_name||null,raw_phones:person.rawPhones||[],review_reasons:review?.reasons||[],review_suggestion:review?.suggestion||null,program_name:programName,candidate_ids:candidates.map(x=>x.id||x).filter(Boolean),review_created_at:now};
 const truth={status:'needs_decision',source:'scan',updated_at:now,candidate_ids:candidates.map(x=>x.id||x).filter(Boolean)};
 const result=await client.query(`INSERT INTO people(organization_id,first_name,last_name,display_name,phone,phone_numbers,type,status,confidence,source,created_by,last_scan_job_id,quarantine_reason,living_truth,metadata) VALUES($1,$2,$3,$4,$5,$6,'visitor','active',$7,'scan',$8,$9,$10,$11,$12) RETURNING id,display_name,phone,phone_numbers`,[orgId,person.first_name,person.last_name,person.display_name,person.phone,JSON.stringify(phones),normalizeConfidence(person.confidence,70),actorId,jobId,reason,JSON.stringify(truth),JSON.stringify(metadata)]);
...
export async function processVisionJob(jobId,imageBase64,orgId,programName,options={}){
 ...
 try{
  ...
  if(!evaluation){
   ...
   if(cached.rows.length){await updateJobState(client,jobId,INTERNAL_STATES.COMPLETED,'complete',cached.rows[0].result,0,'groq',startTime);return cached.rows[0].result}
  }
  if(!evaluation)await updateJobState(client,jobId,INTERNAL_STATES.EXTRACTING,'understanding_page',null,0,'groq',startTime);
  await client.query(`SELECT pg_advisory_lock(hashtextextended('nyeocare:groq:scan',0))`);groqLocked=true;
  let vision;
  ...
  finally{await client.query(`SELECT pg_advisory_unlock(hashtextextended('nyeocare:groq:scan',0))`).catch(()=>{});groqLocked=false}
  const rawContent=vision?.data?.choices?.[0]?.message?.content;
  if(typeof rawContent!=='string'||!rawContent.trim())return errorResult('ai_request','EMPTY_AI_RESPONSE','Vision returned no content.','ARIA could not safely read the register.');
  ...
  if(!validation.valid)return errorResult('parse_json','INVALID_AI_RESPONSE',validation.error,'ARIA could not safely understand the register.');
  ...
  const newPeople=[],matchedPeople=[],reviewPeople=[],events=[];
  for(const person of validation.people){
   const review=reviewFor(person,validation);
   if(review){
    const candidate=await insertPendingPerson(client,orgId,jobId,actorId,programName,person,review);
    reviewPeople.push({id:candidate.id,name:candidate.display_name,phone:candidate.phone,phones:candidate.phone_numbers||[],reason:review.reason,suggestion:review.suggestion});
    continue;
   }
   ...
   if(decision?.status==='conflict'||decision?.status==='needs_decision'){
    const review={reason:decision.status==='conflict'?'ARIA found more than one possible existing person.':'ARIA needs more evidence before confirming this identity.',suggestion:decision.status==='conflict'?'Review the possible matches and decide what this scan means.':'Review the original register and confirm the identity.',reasons:[decision.status==='conflict'?'identity_conflict':'identity_needs_decision']};
    const candidate=await insertPendingPerson(client,orgId,jobId,actorId,programName,person,review,decision.candidates||[]);
    reviewPeople.push({id:candidate.id,name:candidate.display_name,phone:candidate.phone,phones:candidate.phone_numbers||[],reason:review.reason,suggestion:review.suggestion});
    continue;
   }
   const type=['regular','returning','familiar_face'].includes(person.relationship_stage)?'member':'visitor',phoneNumbers=phoneRecords(person),metadata={scan_pipeline_version:PIPELINE_VERSION,honorific:person.honorific||null,row_number:person.row_number,scan_evidence_job_id:jobId,vision_provider:'groq',phone_numbers:phoneNumbers,raw_name:person.raw_name||null,raw_phones:person.rawPhones||[]};
   const insert=await client.query(`INSERT INTO people(organization_id,first_name,last_name,display_name,phone,phone_numbers,type,status,confidence,source,created_by,last_scan_job_id,living_truth,metadata) VALUES($1,$2,$3,$4,$5,$6,$7,'active',$8,'scan',$9,$10,$11,$12) RETURNING id,display_name,phone,phone_numbers,type`,[orgId,person.first_name,person.last_name,person.display_name,person.phone,JSON.stringify(phoneNumbers),type,normalizeConfidence(person.confidence,70),actorId,jobId,JSON.stringify({status:'alive',source:'scan',updated_at:new Date().toISOString()}),JSON.stringify(metadata)]);
  ...
  const totalPeople=validation.people.length,result={status:'ok',scan_pipeline_version:PIPELINE_VERSION,new_members:newPeople.length,updated:matchedPeople.length,duplicates:[],needs_review:reviewPeople,rejected:validation.rejected,people:validation.people.map(p=>({name:p.display_name,phone:p.phone,phones:p.phones||[],raw_phones:p.rawPhones||[],review:Boolean(reviewFor(p,validation))})),total_extracted:totalPeople,total_valid:totalPeople,register_mode:registerMode,verification:{accepted:validation.people.length-reviewPeople.length,review:reviewPeople.length,passes:vision.verification?.passes||1},evidence:{available:true,job_id:jobId},layout:{provider:'groq-direct-vision',rows:totalPeople},summary:`ARIA found ${totalPeople} people, remembered ${newPeople.length+matchedPeople.length}, and sent ${reviewPeople.length} to Review Center.`};
  ...
  await pool.query(`UPDATE scan_jobs SET status='complete',progress='complete',completed_at=NOW(),last_progress_at=NOW(),heartbeat=NOW(),attempt_count=$2,provider_used='groq',result=$3 WHERE id=$1`,[jobId,vision.attempt,result]);
  return result;
 }catch(err){
  ...
  return e;
 }finally{if(groqLocked)await client.query(`SELECT pg_advisory_unlock(hashtextextended('nyeocare:groq:scan',0))`).catch(()=>{});try{client.release()}catch{}}
 }
