...
export default withAdmin(async function handler(req, res) {
  ...
  try {
    ...
    await client.query(
      ...
      "WHERE organization_id=$2 AND session_id=$3 AND present=true AND confirmed=false",
      [userId, orgId, session_id]
    );

    const closed = await client.query(
      "UPDATE sessions SET status='closed',closed_by=$1,closed_at=NOW()," +
      "aria_processing_status='processing',aria_processing_attempts=aria_processing_attempts+1," +
      "aria_processing_started_at=NOW(),aria_processing_completed_at=NULL,aria_processing_error=NULL " +
    ...
    waitUntil((async () => {
      try {
        await generateParticipationFromSession(session_id, orgId);
        await pool.query(
        ...
        console.error('[ATTENDANCE] ARIA processing failed after close:', e);
        try {
          await pool.query(
          ...
          const event = await emitAriaEvent({
            ...
            metadata: {
              session_id,
              session_name: closed.rows[0]?.name || null,
              error: internal
            },
            eventKey: 'attendance:' + session_id + ':aria_failed:' + (closed.rows[0]?.aria_processing_attempts || 1)
          });
          if (event) await directAriaEvent(event);
        } catch (updateError) {
          console.error('[ATTENDANCE] Could not persist processing failure:', updateError);
        }
      }
    })());
    ...
    return res.status(200).json({
      ...
      error: null,
      session: { ...closed.rows[0], aria_processing_status: 'processing' },
      message: 'Attendance saved. ARIA is now finishing the intelligence pass.'
    });
  } catch (err) {
