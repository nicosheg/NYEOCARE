...
export default withAdmin(async function handler(req,res){
 ...
 try{
  ...
  const q=await client.query('SELECT id,name,status,started_at FROM sessions WHERE id=$1 AND organization_id=$2 LIMIT 1 FOR UPDATE',[session_id,orgId]);
  if(!q.rows.length){await client.query('ROLLBACK');return res.status(404).json({error:'Attendance session not found.'});}
  if(q.rows[0].status!=='active'){await client.query('ROLLBACK');return res.status(409).json({error:'This attendance session is already saved.'});}
