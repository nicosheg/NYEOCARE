...
if(typeof present!=='boolean')return res.status(400).json({error:'present must be boolean.'});
...
if(!present){
if(existing.rows[0]&&!existing.rows[0].confirmed)await client.query(`DELETE FROM attendance_records WHERE id=$1 AND organization_id=$2 AND confirmed=false`,[existing.rows[0].id,orgId]);
...
if(!row){
if(!present){await client.query('COMMIT');return res.status(200).json({success:true,present:false,confirmed:true,changed:false,person:person.rows[0]})}
const inserted=await client.query(`INSERT INTO attendance_records(people_id,attendance_date,present,session_id,marked_by,marked_at,status,confirmed,reviewed_by,reviewed_at,organization_id) VALUES($1,$2,true,$3,$4,NOW(),'present',true,$4,NOW(),$5) RETURNING id,marked_at`,[people_id,new Date(session.rows[0].started_at||Date.now()).toISOString().slice(0,10),session_id,userId,orgId]);
await client.query('COMMIT');
