...
export default withAdmin(async function handler(req,res){
 ...
 try{return res.status(200).json({success:true,aria:await generateParticipationFromSession(session_id,req.org.id)});}catch(e){console.error('[ATTENDANCE] Retry ARIA processing:',e);return res.status(500).json({error:e.message||'Unable to process attendance session.'});}
});
