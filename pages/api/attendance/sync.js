...
export default withOrg(async function handler(req,res){
 ...
 const orgId=req.org.id,userId=req.user.id,isAdmin=['owner','admin'].includes(String(req.user.role||'').toLowerCase()),client=await pool.connect();
 try{
  ...
  const validSet=new Set(valid),ignored=ids.filter(id=>!validSet.has(id)),ops=[...deduped.values()].filter(x=>validSet.has(x.people_id)),payload=JSON.stringify(ops),date=new Date(session.started_at||Date.now()).toISOString().slice(0,10),confirmed=session.status==='closed'&&isAdmin;
  if(ops.length){
  ...
  return res.status(200).json({success:true,applied_people_ids:valid,ignored_people_ids:ignored,reprocessing_queued:reprocessingQueued,pending_confirmation_people_ids:pendingConfirmation});
