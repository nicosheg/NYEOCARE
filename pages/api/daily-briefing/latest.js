...
priority:110,
label:'SCAN REVIEW',
title:`${reviewCount} register row${reviewCount===1?'':'s'} need confirmation`,
message:`ARIA found information it could not safely verify. Confirm the name and phone against the original register before saving them.`,
...
if(!name)continue;
const message=String(x.evidence?.summary||x.metadata?.summary||'').trim();
if(!message)continue;
items.push({
id:x.id,
person_id:x.person_id,
category:'care',
priority:Math.max(priority(x.urgency),Number(x.attention_score)||0),
label:cleanType(x.type)||'FOLLOW-UP',
title:name,
message,
action:{type:'people',label:'Open person',href:'/people'}
});
}

for(const x of actions.rows){
const name=personName(x);
const message=String(x.action_metadata?.summary||x.action_metadata?.message||'').trim();
if(!message)continue;
items.push({
id:x.id,
person_id:x.person_id,
...
label:cleanType(x.type)||'ACTION',
title:name||'Action needed',
message,
action:{type:'people',label:'Open people',href:'/people'}
});
}
...
items.push({
id:`relationship:${x.person_id}`,
person_id:x.person_id,
category:'relationship',
priority:70+(45-score),
label:'RELATIONSHIP',
title:name,
message:`Relationship signal is low at ${Math.round(score)}%. Open this person’s record and decide whether a personal follow-up is needed.`,
action:{type:'people',label:'Open person',href:'/people'}
});
}
...
categories:{
scan:top.filter(x=>x.category==='scan').slice(0,1),
care:top.filter(x=>x.category==='care').slice(0,3),
relationship:top.filter(x=>x.category==='relationship').slice(0,2)
},
