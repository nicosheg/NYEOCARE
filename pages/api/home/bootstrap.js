...
export default withOrg(async function handler(req,res){
 ...
 try{
  const[org,people,review,session,observations,actions]=await Promise.all([
   ...
   pool.query("SELECT COUNT(*)::int count FROM scan_review_items WHERE organization_id=$1 AND status='pending'",[orgId]),
  ...
  const settings=org.rows[0]?.settings||{},vocabulary=settings?.aria?.vocabulary||{person:'people',members:'members',leaders:'leaders',care:'care',prayer:'prayer'},now=new Date(),date=now.toISOString().slice(0,10),count=top.length,s=session.rows[0]||null;
  const director={director:'ARIA',version:ARIA_DIRECTOR_VERSION,people:Number(people.rows[0]?.count)||0,pending_scan_reviews:Number(review.rows[0]?.count)||0,latest_session:s?{id:s.id,name:s.name,status:s.status,aria_processing_status:s.aria_processing_status,aria_processing_error:s.aria_processing_error}:null,active_observations:observations.rows.length,open_actions:actions.rows.length};
  return res.status(200).json({date,generatedAt:now.toISOString(),director,organization:{id:orgId,name:org.rows[0]?.name||'your organization',instructions:org.rows[0]?.aria_instructions||'',vocabulary},attendance:s?{active:s.status==='active',recoverable:s.status==='closed',session_id:s.id,name:s.name,status:s.status,started_by:s.started_by,started_at:s.started_at,closed_at:s.closed_at,processing_status:s.aria_processing_status,processing_attempts:Number(s.aria_processing_attempts)||0,processing_started_at:s.aria_processing_started_at,processing_completed_at:s.aria_processing_completed_at,processing_error:s.aria_processing_error}:{active:false,recoverable:false},notification:{hasSomething:count>0,text:count?'ARIA has concrete things for you today.':'ARIA is keeping watch today.',count},briefing:{headline:count?'Here are '+count+' things with a clear next step.':'Nothing needs your immediate attention today.',items:top},categories:{scan:[],care:top.filter(x=>x.category==='care').slice(0,3)},peopleCount:Number(people.rows[0]?.count)||0,reviewCount:Number(review.rows[0]?.count)||0,nextRefresh:date+'T23:59:59.999'});
