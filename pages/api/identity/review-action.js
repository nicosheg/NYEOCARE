...
const parts=splitName(cleanName);
const normalizedPhone=new_phone?normalizePhone(String(new_phone).trim()):null;
const p=await client.query(`INSERT INTO people(organization_id,first_name,last_name,display_name,phone,type,status,confidence,source,created_by,last_scan_job_id,living_truth,metadata) VALUES($1,$2,$3,$4,$5,'visitor','active',90,'scan',$6,$7,$8,$9) RETURNING id`,[orgId,parts.first_name,parts.last_name,cleanName,normalizedPhone,req.user.id,scan_job_id,JSON.stringify({status:'alive',source:'human_review',updated_at:new Date().toISOString()}),JSON.stringify({resolved_from_scan_review:true})]);
