...
['conversations',`SELECT c.id,c.status,c.created_at,c.updated_at,COALESCE(json_agg(json_build_object('id',m.id,'role',m.role,'content',m.content,'metadata',m.metadata,'created_at',m.created_at) ORDER BY m.created_at) FILTER(WHERE m.id IS NOT NULL),'[]'::json) AS messages FROM aria_conversations c LEFT JOIN aria_messages m ON m.conversation_id=c.id WHERE c.organization_id=$1 AND c.person_id=$2 GROUP BY c.id ORDER BY c.updated_at DESC`,[orgId,id]],
