...
import pool from'../../../lib/db';import{withOrg}from'../../../lib/apiHelpers';import{detectDuplicates}from'../../../lib/duplicateDetector';import{normalizeName}from'../../../lib/scanValidation';import{normalizePhoneList}from'../../../lib/phoneUtils';
...
const merged=[...scanItems,...attendanceItems,...groups].sort((a,b)=>Number(b.score||0)-Number(a.score||0));return res.status(200).json({items:merged,total:merged.length,pending_count:scanItems.length+attendanceItems.length,scan_pending_count:scanItems.length,attendance_review_count:attendanceItems.length,database_duplicate_count:groups.length,people_count:audit.people_count,verified_count:audit.verified_count,unverified_count:audit.unverified_count,detector_version:audit.detector_version});}catch(err){console.error('[REVIEW] List error:',err);return res.status(500).json({error:'Review Center could not load.'})}});
