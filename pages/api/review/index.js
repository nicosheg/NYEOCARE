...
export default withOrg(async function handler(req,res){
 ...
 try{
  ...
  const items=rows.map(x=>({id:x.id,name:x.display_name||[x.first_name,x.last_name].filter(Boolean).join(' '),first_name:x.first_name,last_name:x.last_name,phone:x.phone,phones:x.phone_numbers||[],type:x.type,status:x.living_truth?.status||'needs_decision',reason:x.quarantine_reason||'ARIA found something that needs your attention.',suggestion:x.metadata?.review_suggestion||'Review the original register and decide what is true.',raw_name:x.metadata?.raw_name||null,raw_phones:x.metadata?.raw_phones||[],row_number:x.metadata?.row_number??null,scan_job_id:x.last_scan_job_id,candidates:x.metadata?.candidate_ids||[],review_reasons:x.metadata?.review_reasons||[],created_at:x.created_at,updated_at:x.updated_at}));
