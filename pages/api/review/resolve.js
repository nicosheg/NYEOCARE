...
function splitName(value){
 const n=normalizeDisplayName(value),parts=n.core.split(/\s+/).filter(Boolean);
 return{first_name:parts.shift()||'',last_name:parts.join(' '),display:n.display};
}
function phoneList(body,person){
 const raw=Array.isArray(body?.phones)?body.phones:(body?.phone?[body.phone]:[]);
 if(raw.length)return raw.map(x=>normalizePhone(x)).filter(Boolean).slice(0,2);
 if(Array.isArray(person.phone_numbers))return person.phone_numbers.map(x=>typeof x==='string'?x:x?.normalized||x?.raw).filter(Boolean).slice(0,2);
 return person.phone?[person.phone]:[];
}
async function handler(req,res){
 ...
 try{
  ...
  const found=await client.query(`SELECT * FROM people WHERE id=$1 AND organization_id=$2 AND status='active' AND living_truth->>'status' IN ('needs_decision','conflict') FOR UPDATE`,[id,req.org.id]);
  ...
  if(normalizedPhone){
   ...
   if(duplicate.rows.length){await client.query('ROLLBACK');return res.status(409).json({error:'That phone number already belongs to another active person.',code:'DUPLICATE_ACTIVE_PERSON',person:duplicate.rows[0]})}
  }
  const actionName=action==='edit'?'edited':'approved';
  const truth={...(person.living_truth||{}),status:'alive',source:'human_review',confirmed_at:new Date().toISOString(),confirmed_by:req.user.id};
  const metadata={...(person.metadata||{}),review_action:actionName,reviewed_by:req.user.id,reviewed_at:new Date().toISOString()};
  ...
  const event=await emitAriaEvent({organizationId:req.org.id,personId:id,type:'PERSON_UPDATED',source:'human',actorId:req.user.id,metadata:{review_action:actionName,review_id:id},eventKey:`review:${id}:${actionName}:${Date.now()}`},client);
  await client.query('COMMIT');
  if(event)processAriaEvent(event).catch(()=>{});
