...
async function handler(req,res){
 ...
 if(!admission.allowed)return res.status(admission.code==='AI_NOT_CONFIGURED'?503:429).json({error:admission.message,code:admission.code,retry_after_seconds:admission.retry_after_seconds||null});
 ...
 try{
  ...
  res.status(200).json({job_id:jobId,status:'pending',progress:'queued',pipeline:'v12-reliable-compact-vision',token_plan:{primary_max:1200,recovery_max:1000,safety_validation:'server'}});
  Promise.resolve(processVisionJob(jobId,image_base64,orgId,programName,{actorId,registerMode:'complete'})).catch(err=>console.error(`[SCAN] Background job ${jobId} failed:`,err?.message||err));
 }catch(err){
