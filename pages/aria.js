...
export default function AriaPage(){
 ...
 const[suggestion,setSuggestion]=useState(null),[draft,setDraft]=useState(null),[drafts,setDrafts]=useState([]),[daily,setDaily]=useState(null),[todayOpen,setTodayOpen]=useState(false);
 ...
 const send=async(value)=>{
  ...
  try{
   ...
   if(mounted.current){
    ...
    setSuggestion(d.suggestedAction||null);setDraft(d.draft||null);setDrafts(Array.isArray(d.drafts)?d.drafts:[]);
   }
   loadRecent();
  }catch(e){
 ...
 return <Layout><main className="ariaPage">
  ...
  <section className="stage">
   ...
   <div className={"thread "+((messages.length||busy||error||suggestion||draft)?"interactive":"passive")} ref={threadRef}>{messages.map((m,i)=><article className={'message '+m.role} key={(m.id||m.at)+'-'+i}><small>{m.role==='assistant'?'ARIA':'YOU'}</small><div>{m.role==="assistant"?<MarkdownMessage content={m.content}/>:m.content}</div></article>)}{busy&&<div className="thinking"><span className="thinkingOrb">A</span><span>ARIA is thinking…<small>Using only the context needed for this question.</small></span></div>}{error&&<div className="error">{error}</div>}{suggestion&&<div className="next"><div><small>{suggestion.status==='awaiting_confirmation'?'CONFIRM WITH ARIA':'NEXT ACTION'}</small><strong>{suggestion.label||'ARIA suggests a next step'}</strong><span>{suggestion.reason||'Nothing happens without your approval.'}</span></div><div className="nextActions">{suggestion.status==='awaiting_confirmation'?<><button onClick={()=>send('Yes, prepare it.')} disabled={busy}>Prepare it</button><button onClick={()=>send('No, not now.')} disabled={busy}>Not now</button></>:suggestion.approved===true&&suggestion.actionId&&suggestion.actionType==='SEND_MESSAGE'?<button onClick={draftMessage} disabled={busy}>Draft message</button>:suggestion.personId?<button onClick={approve} disabled={busy}>{suggestion.actionId?'Approve action':'Approve next step'}</button>:null}</div></div>}{draft&&<div className="draft"><small>DRAFT MESSAGE</small><p>{draft.message}</p><div className="draftActions">{draft.whatsappUrl?<a href={draft.whatsappUrl} target="_blank" rel="noreferrer">Review in WhatsApp ↗</a>:<span>No phone number is recorded for WhatsApp.</span>}<button onClick={()=>{navigator.clipboard?.writeText(draft.message||'').catch(()=>{});setError('Draft copied to clipboard.')}}>Copy</button></div></div>}{drafts.length>0&&<div className="draft batchDrafts"><small>WHATSAPP DRAFTS · {drafts.length}</small>{drafts.map((d,i)=><div className="batchDraft" key={d.personId||i}><div><b>{d.name||'Person'}</b><p>{d.message}</p></div><div className="draftActions">{d.whatsappUrl?<a href={d.whatsappUrl} target="_blank" rel="noreferrer">Open WhatsApp ↗</a>:<span>No phone</span>}<button onClick={()=>{navigator.clipboard?.writeText(d.message||'').catch(()=>{});setError('Draft copied to clipboard.')}}>Copy</button></div></div>)}</div>}</div>
  </section>
