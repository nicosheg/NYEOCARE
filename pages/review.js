...
export default function ReviewPage(){
 ...
 async function load(){
  ...
  try{
   ...
   const r=await fetch('/api/review',{headers:{Authorization:`Bearer ${session.access_token}`}}),d=await r.json();
 ...
 return <main style={s.page}>
  <section style={s.shell}>
   ...
   <div style={s.stack}>
    {items.map((item,i)=><button key={item.id} style={{...s.card,animation:`reviewIn .35s ease ${i*.035}s both`}} onClick={()=>open(item)}>
