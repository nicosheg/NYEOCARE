...
const checks=[
 ...
 ['People roster API projects canonical attendance last_seen field',
  /last_attended_date/.test(peopleApi)&&/r\.last_attended_date/.test(peopleApi)],
 ['People cards use Last seen as the human-facing last-attendance label',
  /const formatLastSeen=/.test(peoplePage)&&/Last seen ·/.test(peoplePage)&&/formatLastSeen\(person\.last_attended_date\)/.test(peoplePage)&&!/formatLastAttended\(/.test(peoplePage)],
 ['Person journey exposes last attendance and last interaction separately',
  /last_attended_date/.test(personPage)&&/last_interaction_at/.test(personPage)&&/Last interaction/.test(personPage)],
 ['Person journey computes interaction from human evidence only',
  /WITH last_interaction AS/.test(read('pages/api/person/journey.js'))&&/person_communications/.test(read('pages/api/person/journey.js'))&&/care_feedback/.test(read('pages/api/person/journey.js'))&&/aria_care_contexts/.test(read('pages/api/person/journey.js'))&&/source,'human'/.test(read('pages/api/person/journey.js'))],
 ['ARIA attendance review has one-tap human attendance correction',
  /actionId/.test(ariaContextAction)&&/attendance/.test(ariaContextAction)&&/attendance_records/.test(ariaContextAction)&&/participation_records/.test(ariaContextAction)&&/status='handled'/.test(ariaContextAction)],
 ['ARIA attendance correction refreshes derived intelligence',
  /refreshAttendanceIntelligence/.test(ariaContextAction)&&/resolution:'attendance_confirmed'/.test(ariaContextAction)&&/resolution:'absence_confirmed'/.test(ariaContextAction)],
 ['ARIA care engine throttles duplicate proactive prompts using last interaction',
  /last_interactions AS/.test(read('lib/aria/careEngine.js'))&&/last_interaction_at/.test(read('lib/aria/careEngine.js'))&&/last_interaction_at<NOW\(\)-INTERVAL '14 days'/.test(read('lib/aria/careEngine.js'))],
