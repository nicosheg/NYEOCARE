...
CREATE POLICY session_sections_insert_admin ON public.session_sections
  FOR INSERT TO authenticated
  WITH CHECK ((organization_id = current_user_organization_id()) AND is_admin());

CREATE POLICY session_sections_update_admin ON public.session_sections
  FOR UPDATE TO authenticated
  USING ((organization_id = current_user_organization_id()) AND is_admin())
  WITH CHECK ((organization_id = current_user_organization_id()) AND is_admin());

CREATE POLICY session_sections_delete_admin ON public.session_sections
  FOR DELETE TO authenticated
  USING ((organization_id = current_user_organization_id()) AND is_admin());

CREATE POLICY session_users_insert_self_or_admin ON public.session_users
  FOR INSERT TO authenticated
  WITH CHECK (
    EXISTS (
    ...
    AND (user_id=current_app_user_id() OR is_admin())
  );

CREATE POLICY attendance_records_update_collaborative ON public.attendance_records
  FOR UPDATE TO authenticated
  USING (
    organization_id=current_user_org_id()
    AND (is_admin() OR (marked_by=current_app_user_id() AND confirmed=false))
  )
  WITH CHECK (
    organization_id=current_user_org_id()
    AND (is_admin() OR (marked_by=current_app_user_id() AND confirmed=false))
...
CREATE POLICY attendance_records_delete_collaborative ON public.attendance_records
  FOR DELETE TO authenticated
  USING (
    organization_id=current_user_org_id()
    AND (is_admin() OR (marked_by=current_app_user_id() AND confirmed=false))
